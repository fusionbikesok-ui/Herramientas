...
export function preparacionRouter(db, cfg) {
  ...
  router.get('/pendientes', (req, res) => {
    try {
      const rows = db.prepare("SELECT * FROM pedidos_cache WHERE estado_envio='pendiente' ORDER BY CASE WHEN canal='ml' THEN 0 ELSE 1 END, fecha ASC").all();
  ...
  router.post('/:id/foto', (req, res, next) => {
    ...
    const uploadKey = uploadId ? String(uploadId) : null;
    if (uploadKey) {
      ...
      if (previa) return res.json({ ok: true, foto: previa, idempotente: true });
    }
