...
export async function refrescarPublicacionesMl(db, cfg, onProgress) {
  ...
  for (let i = 0; i < allIds.length; i += MULTIGET_CHUNK) {
    ...
    const resp = await mlFetchConReintento(
      db, cfg, 'get',
      `/items/bulk?ids=${chunk.join(',')}&include_attributes=all&attributes=id,title,status,sub_status,seller_custom_field,attributes,variations,secure_thumbnail,thumbnail,permalink,catalog_listing,catalog_product_id,price,available_quantity,user_product_id,channels,category_id,listing_type_id,shipping,date_created`,
      null, { manual: true }
    );
