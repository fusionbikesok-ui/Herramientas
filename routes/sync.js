...
async function _reconciliarStockMl(db, cfg) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const path = `/items/bulk?ids=${chunk.join(',')}&attributes=id,status,sub_status,available_quantity,variations`;
    let resp;
    try {
...
async function evaluarPreciosReactivables(db, mlCfg, filasPorItem) {
  ...
  await mapConLimite(chunks, ML_CONCURRENCIA_MAX, async (chunk) => {
    try {
      const resp = await mlFetch(
        db, mlCfg, 'get',
        `/items/bulk?ids=${chunk.join(',')}&attributes=id,price,category_id,listing_type_id,shipping,variations`
      );
      if (resp.status === 200 && Array.isArray(resp.data)) {
...
export async function reactivarItems(db, mlCfg, itemIds, opts = {}) {
  ...
  await mapConLimite(chunks, ML_CONCURRENCIA_MAX, async (chunk) => {
    try {
      const resp = await mlFetch(
        db, mlCfg, 'get',
        `/items/bulk?ids=${chunk.join(',')}&attributes=id,status,sub_status,price,category_id,listing_type_id,shipping,variations`,
        null, opts
      );
      if (resp.status === 200 && Array.isArray(resp.data)) {
        for (const e of resp.data) if ((e.status_code ?? e.code) === 200 && e.body) items.set(String(e.body.id), e.body);
...
async function variacionesVivasDeMl(db, mlCfg, itemIds) {
  ...
  for (let i = 0; i < ids.length; i += MULTIGET_CHUNK) {
    ...
    const resp = await mlFetch(db, mlCfg, 'get', `/items/bulk?ids=${chunk.join(',')}&attributes=id,status,variations`);
    await sleep(espera(ML_CALL_DELAY_MS));
...
async function enriquecerConMl(db, mlCfg, rows) {
  ...
  for (let i = 0; i < ids.length; i += MULTIGET_CHUNK) {
    ...
    const resp = await mlFetch(db, mlCfg, 'get',
      `/items/bulk?ids=${chunk.join(',')}&attributes=id,title,secure_thumbnail,thumbnail`);
...
async function diagnosticarErrores(db, mlCfg, rows) {
  ...
  for (let i = 0; i < itemIds.length; i += MULTIGET_CHUNK) {
    ...
    const resp = await mlFetch(db, mlCfg, 'get',
      `/items/bulk?ids=${chunk.join(',')}&attributes=id,title,status,sub_status,variations,secure_thumbnail,thumbnail`);
    if (resp.status !== 200 || !Array.isArray(resp.data)) continue;
    for (const entry of resp.data) {
