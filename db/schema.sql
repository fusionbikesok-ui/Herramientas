...
CREATE TABLE IF NOT EXISTS recepcion_items (
  ...
  estado_item      TEXT,
  -- NULL/'pendiente' (borrador) | 'aplicando' | 'aplicado' | 'error_reintentable' | 'operacion_incierta' |
  -- 'conflicto_stock' | 'sin_match' | 'no_recibido' | 'pendiente_creacion' | 'creado'
  -- (P0.1, máquina de estados durable de la aplicación de stock — ver routes/recepciones.js)
  ficha_json       TEXT,   -- ficha Gemini para pendiente_creacion (productos nuevos)
  error_wc         TEXT,   -- mensaje de error/conciliación si estado_item='error_reintentable'|'operacion_incierta'|'conflicto_stock'
  resuelto_en      TEXT,
  creado_en        TEXT NOT NULL,
  operation_id     TEXT,    -- token del intento de aplicación en curso/último; guarda cada UPDATE contra clobbers cruzados
  stock_objetivo   INTEGER, -- stock esperado tras el PATCH, persistido ANTES de dispararlo (para conciliar sin adivinar)
  aplicando_desde  TEXT     -- timestamp de inicio del intento 'aplicando' (diagnóstico)
);

CREATE TABLE IF NOT EXISTS recepcion_aliases_proveedor (
  id INTEGER PRIMARY KEY AUTOINCREMENT, proveedor_norm TEXT NOT NULL, codigo_norm TEXT NOT NULL DEFAULT '',
  descripcion_norm TEXT NOT NULL, variacion_norm TEXT NOT NULL DEFAULT '', id_woo INTEGER NOT NULL,
  sku TEXT, recepcion_item_id INTEGER, creado_por TEXT NOT NULL, vigente_desde TEXT NOT NULL,
  vigente_hasta TEXT, motivo_cierre TEXT,
  CHECK (codigo_norm <> '' OR descripcion_norm <> ''), CHECK ((vigente_hasta IS NULL) = (motivo_cierre IS NULL))
);
