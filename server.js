...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  import('dotenv/config').then(() => {
    ...
    if (process.env.DISABLE_CRONS === 'true') {
      ...
      // Renovación proactiva del token ML. getAccessToken solo pega a ML cuando faltan
      // <30min para el vencimiento (o está en cooldown de backoff, en cuyo caso no hace
      // nada) — este cron cada 30min es lo que da la oportunidad regular de renovar antes
      // de que el token llegue a vencer, sin sumar otro llamador más a la tormenta que
      // causó el incidente original.
      cron.schedule('*/30 * * * *', () => {
