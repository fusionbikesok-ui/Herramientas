 ...
 */
// Estados de envío de ML que significan que el paquete ya salió. `ready_to_ship` no entra:
// está listo pero todavía no despachado, que es precisamente lo que hay que seguir viendo.
...
function estadoEnvioMl(db, shipmentId) {
  ...
  try {
    return db.prepare('SELECT status FROM ml_shipment_estado WHERE shipment_id=?').get(String(shipmentId))?.status || null;
...
function estadoOperativoAPersistir(orden, existente, envioMl) {
  ...
  if (envioMl && ENVIOS_ML_DESPACHADOS.includes(envioMl)) return 'cerrado';
