...
export function upsertGestionPedido(db, orden, options = {}) {
  ...
  if (existente) {
    ...
    db.prepare(`UPDATE gestion_pedidos SET cliente_id=?, numero_visible=?, estado_comercial=?, estado_operativo=?, pago_estado=?, pago_metodo=?, cuotas=?, notas=?, cancelado_en=?, actualizado_en=? WHERE id=?`)
      .run(clienteId, orden.numero || idExterno, comercial, operativo, orden.pago_estado || null, orden.pago_metodo || null, orden.cuotas ?? null, orden.notas || null, comercial === 'cancelado' ? (orden.cancelado_en || timestamp) : null, timestamp, pedidoId);
  } else {
    pedidoId = db.prepare(`INSERT INTO gestion_pedidos
      (cliente_id, fuente, external_id, numero_visible, estado_comercial, estado_operativo, pago_estado, pago_metodo, cuotas, notas, creado_fuente_en, cancelado_en, importado_en, actualizado_en)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(clienteId, fuente, idExterno, orden.numero || idExterno, comercial, operativo, orden.pago_estado || null, orden.pago_metodo || null, orden.cuotas ?? null, orden.notas || null, orden.fecha,
