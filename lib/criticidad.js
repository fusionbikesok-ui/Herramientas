...
// Fase 3 (Rotación y criticidad) del plan de control de stock de José — ver
// docs/superpowers/plans/2026-08-plan-jose-control-stock-ciclos.md.
...
// `after` es ISO sin timezone explícita — Woo lo interpreta en el timezone del sitio; no es
// exacto al segundo, por eso el margen de 1 día del cursor de arriba.
  ...
  for (;;) {
    ...
    for (const orden of ordenesPagina) {
      ...
      for (const item of ov.items) {
        ...
        if (!sku && item.seller_sku) sku = item.seller_sku;
        if (!sku) continue;
