...
export async function conciliarAltaIncierta({ db, cfg, operationId, fetchWoo = wooFetch, resolucionManual = null }) {
  ...
  if (!idWoo) {
    // Si es familia_variable sin padre conocido (el POST del padre timeouteo), intentar recuperarlo
    // por su SKU provisional ANTES de intentar buscar la variación.
