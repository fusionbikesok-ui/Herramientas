...
export function validarFichaAlta(input = {}) {
  ...
  if (!Array.isArray(f.atributos) || f.atributos.length===0 || f.atributos.some(a=>!String(a?.nombre||'').trim()||!String(a?.valor||'').trim())) throw new Error('atributos inválidos');
...
export async function crearBorradorWoo({db,cfg,operationId,ficha,actor,fetchWoo=wooFetch}) {
  ...
  if (f.modo === 'familia_variable' || f.modo === 'variacion_existente') {
    ...
    if (f.parent_id) {
      const variacionesExistentes = (await fetchWoo(cfg, `/products/${f.parent_id}/variations?per_page=100`)).data || [];
...
/**
 * P1.6: recuperación de una alta 'incierto'. Lee Woo (por el id_woo que haya quedado persistido
 * durante el intento fallido) para decidir sin adivinar: si el recurso existe con el SKU y draft
 * esperados, la operación pasa a 'creado' (y se hace el upsert en catalogo_cache, igual que un
 * alta exitosa). Si Woo confirma que NO existe, pasa a 'fallido', que es el único estado que
 * libera reintento (ver comentario en routes/recepciones.js sobre el claim de /crear-alta).
 *
 * Si el intento incierto nunca llegó a conocer un id_woo (el timeout ocurrió en el primer POST,
 * antes de recibir respuesta), no hay nada verificable por id ni por SKU — el SKU se deriva del
 * id, así que tampoco hay nada que buscar por SKU. Fail-closed: se deja 'incierto' explícitamente
 * para revisión manual, nunca se asume que no se creó nada sin evidencia.
 */
export async function conciliarAltaIncierta({ db, cfg, operationId, fetchWoo = wooFetch }) {
  ...
  if (!row.id_woo) return { estado: 'incierto', motivo: 'sin id_woo conocido: revisar manualmente en Woo' };

  const path = row.modo === 'simple' ? `/products/${row.id_woo}` : `/products/${row.id_padre}/variations/${row.id_woo}`;
  const sku = `FB-${row.id_woo}`;
  let data;
  try {
  ...
  if (data && data.sku === sku && data.status === 'draft') {
    const result = { id_woo: row.id_woo, id_padre: row.id_padre, sku, status: 'draft' };
    const tipo = row.modo === 'simple' ? 'simple' : 'variation';
    upsertCatalogoCacheAlta(db, { id_woo: row.id_woo, id_padre: tipo === 'variation' ? row.id_padre : null, tipo, sku, nombre: data.name || '', stock: 0 });
    db.prepare("UPDATE recepcion_altas_woo SET estado='creado',sku=?,respuesta_json=?,actualizado_en=? WHERE operation_id=?")
      .run(sku, JSON.stringify(result), new Date().toISOString(), operationId);
    return { estado: 'creado', ...result };
  }
  db.prepare("UPDATE recepcion_altas_woo SET estado='fallido',error=?,actualizado_en=? WHERE operation_id=?")
    .run('conciliado: SKU/estado no coinciden con lo esperado', new Date().toISOString(), operationId);
  return { estado: 'fallido' };
}
