...
async function buscarIdPorMarca({ cfg, fetchWoo, modo, idPadre, operationId }) {
  ...
  try {
    if (modo === 'simple') {
      ...
      // Validación defensiva — si hay múltiples resultados para un SKU que debería ser único,
      // tratarlo como no encontrado. Woo filtra por SKU en la query, así que múltiples resultados
      // indican inconsistencia de datos: mejor no adivinar cuál es el recurso correcto.
      if (results.length > 1) {
        console.warn(`[buscarIdPorMarca] ${results.length} resultados para SKU ${sku} (modo simple): inconsistencia de datos, tratando como no encontrado`);
        return null;
      }
    ...
    // Validación defensiva — si hay múltiples resultados para un SKU que debería ser único,
    // tratarlo como no encontrado. Woo filtra por SKU en la query, así que múltiples resultados
    // indican inconsistencia de datos: mejor no adivinar cuál es la variación correcta.
    if (results.length > 1) {
      console.warn(`[buscarIdPorMarca] ${results.length} resultados para SKU ${sku} (modo con padre ${idPadre}): inconsistencia de datos, tratando como no encontrado`);
      return null;
    }
    return results[0]?.id ?? null;
  } catch (_e) {
...
export async function crearBorradorWoo({db,cfg,operationId,ficha,actor,recepcionId=null,recepcionItemId=null,fetchWoo=wooFetch}) {
  ...
  const old=db.prepare('SELECT * FROM recepcion_altas_woo WHERE operation_id=?').get(operationId); if(old){
    // Validación de Tarea 1: rechazar reutilización de operation_id perteneciente a otra recepción/ítem
    // Si old.recepcion_item_id NO es null (fila vieja con vínculo), validar que el vínculo coincida
    if(old.recepcion_item_id !== null && (old.recepcion_id !== recepcionId || old.recepcion_item_id !== recepcionItemId)) throw new Error('operationId pertenece a otra recepción/ítem');
    if(old.request_hash!==hash) throw new Error('operationId reutilizado con ficha distinta'); if(old.estado!=='creado') throw new Error(`operación bloqueada: ${old.estado}`); return JSON.parse(old.respuesta_json);
  }
  ...
  db.prepare(`INSERT INTO recepcion_altas_woo(operation_id,request_hash,estado,modo,creado_por,creado_en,actualizado_en,recepcion_id,recepcion_item_id) VALUES (?,?, 'procesando',?,?,?,?,?,?)`).run(operationId,hash,f.modo,actor,now,now,recepcionId,recepcionItemId);
...
export async function conciliarAltaIncierta({ db, cfg, operationId, fetchWoo = wooFetch, resolucionManual = null }) {
  ...
  try {
    ...
    // 'fallido' SOLO con 404 inequívoco: Woo confirma explícitamente que el id no existe.
    // Cualquier otro error (incluida cualquier variante de mensaje) se propaga sin concluir nada.
    if (e.status === 404) {
      db.prepare("UPDATE recepcion_altas_woo SET estado='fallido',error=?,actualizado_en=? WHERE operation_id=?")
  ...
  // El recurso EXISTE. Si algo no cuadra, es 'incierto' con motivo explícito para que una
  // persona lo resuelva vía resolucionManual — nunca 'fallido', que liberaría reintento.
  if (!data || data.status !== 'draft') {
    const motivo = !data ? 'sin_datos' : `no_draft (status=${data?.status})`;
    db.prepare("UPDATE recepcion_altas_woo SET estado='incierto',error=?,actualizado_en=? WHERE operation_id=?")
      .run(`conciliado: existe pero ${motivo}`, new Date().toISOString(), operationId);
    return { estado: 'incierto', motivo };
  }
  // Validar stock_quantity === 0 y manage_stock === true, igual que crearBorradorWoo
  if (Number(data.stock_quantity || 0) !== 0) {
    db.prepare("UPDATE recepcion_altas_woo SET estado='incierto',error=?,actualizado_en=? WHERE operation_id=?")
      .run(`conciliado: existe pero stock no es cero (stock_quantity=${data.stock_quantity})`, new Date().toISOString(), operationId);
    return { estado: 'incierto', motivo: 'stock_no_cero' };
  }
  if (data.manage_stock !== true) {
    db.prepare("UPDATE recepcion_altas_woo SET estado='incierto',error=?,actualizado_en=? WHERE operation_id=?")
      .run(`conciliado: existe pero manage_stock no está confirmado (manage_stock=${data.manage_stock})`, new Date().toISOString(), operationId);
    return { estado: 'incierto', motivo: 'manage_stock_no_confirmado' };
  }
  if (data.sku !== skuFinal) {
    ...
    if (!data || data.sku !== skuFinal) {
      const motivo = !data ? 'sin_datos_tras_patch' : 'sku_no_confirmado';
      db.prepare("UPDATE recepcion_altas_woo SET estado='incierto',error=?,actualizado_en=? WHERE operation_id=?")
        .run(`conciliado: existe pero no confirmó el SKU final (esperado ${skuFinal}, encontrado ${data?.sku})`, new Date().toISOString(), operationId);
      return { estado: 'incierto', motivo };
    }
  }
