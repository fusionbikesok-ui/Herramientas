...
export async function crearBorradorWoo({db,cfg,operationId,ficha,actor,fetchWoo=wooFetch}) {
  ...
  try {
    ...
    // El padre (variable) de familia_variable NO lleva la marca: no es el recurso final que
    // conciliarAltaIncierta busca (ese es la variación creada debajo). Simple sí, porque para
    // 'simple' el recurso de `payload` ES el final.
...
export async function conciliarAltaIncierta({ db, cfg, operationId, fetchWoo = wooFetch, resolucionManual = null }) {
  ...
  if (!idWoo) {
    idWoo = await buscarIdPorMarca({ cfg, fetchWoo, modo: row.modo, idPadre: row.id_padre, operationId });
  ...
  const path = row.modo === 'simple' ? `/products/${idWoo}` : `/products/${row.id_padre}/variations/${idWoo}`;
  ...
  const result = { id_woo: idWoo, id_padre: row.id_padre, sku: skuFinal, status: 'draft' };
  const tipo = row.modo === 'simple' ? 'simple' : 'variation';
  upsertCatalogoCacheAlta(db, { id_woo: idWoo, id_padre: tipo === 'variation' ? row.id_padre : null, tipo, sku: skuFinal, nombre: data.name || '', stock: 0 });
