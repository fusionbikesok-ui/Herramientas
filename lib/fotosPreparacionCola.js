...
function tomarSiguiente(db) {
  const limite = new Date(Date.now() - LEASE_MS).toISOString();
  db.prepare(`UPDATE preparacion_fotos SET estado_proceso='pendiente', procesando_reclamada_en=NULL
    WHERE estado_proceso='procesando' AND procesando_reclamada_en IS NOT NULL AND procesando_reclamada_en <= ?`).run(limite);
  ...
  const r = db.prepare(`UPDATE preparacion_fotos SET estado_proceso='procesando', procesando_reclamada_en=? WHERE id=? AND estado_proceso='pendiente'`)
    .run(now(), candidata.id);
...
function registrarResultado(db, foto, resultado, urlLiviana) {
  if (resultado.ok) {
    db.prepare(`
      ...
      SET estado_proceso='listo', url_liviana=?, procesado_en=?, ultimo_error=NULL, proximo_intento_en=NULL, procesando_reclamada_en=NULL
      WHERE id=?
  ...
  if (intentos >= MAX_INTENTOS) {
    db.prepare(`
      UPDATE preparacion_fotos SET estado_proceso='error', intentos=?, ultimo_error=?, proximo_intento_en=NULL, procesando_reclamada_en=NULL WHERE id=?
    ...
    db.prepare(`
      UPDATE preparacion_fotos SET estado_proceso='pendiente', intentos=?, ultimo_error=?, proximo_intento_en=?, procesando_reclamada_en=NULL WHERE id=?
...
export function reintentarFoto(db, fotoId) {
  const r = db.prepare(`
    UPDATE preparacion_fotos
    SET estado_proceso='pendiente', intentos=0, ultimo_error=NULL, proximo_intento_en=NULL, procesando_reclamada_en=NULL
