...
const TIPOS = ['informe_faltante', 'entregas_atrasadas', 'plataforma_sin_respuesta', 'informes_ocultos'];
...
/**
 * El vigilante no pudo instalarse (keyring ausente o inválido, destino sin configurar). Abre incidente crítico:
 * fallar en silencio es lo peor que puede hacer justo el componente que existe para detectar que la plataforma
 * dejó de emitir (hallazgo alto de la revisión de T4 del 2026-09-18).
 */
export function anunciarVigilanteApagado(db, motivo) {
  abrirOActualizarIncidente(db, {
    integracion: INTEGRACION, proceso: PROCESO, tipoError: 'vigilante_sin_configuracion', severidad: 'critico',
    mensajeHumano: 'El vigilante de informes no pudo arrancar: nadie va a avisar si la plataforma deja de emitir. '
      + 'Ver docs/superpowers/specs/e1/sop-sombra.md.',
    contexto: { motivo: String(motivo).slice(0, 200) },
  });
}

/**
 * @returns {Promise<{ estado: 'temprano' | 'ok' | 'falta' | 'atrasadas' | 'ocultos' | 'sin_respuesta', esperado?: string }>}
 */
export async function revisarInformeDelDia(db, { url, keyring, fetch: hacerFetch = globalThis.fetch, ahora = new Date(), timeoutMs = 10_000 }) {
  ...
  try {
    ...
    const valida = estado && typeof estado === 'object'
      ...
      // `ocultos` puede faltar si la plataforma es anterior a la migración 0012; si viene, tiene que tener forma.
      && (estado.ocultos === undefined || (Array.isArray(estado.ocultos)
        && estado.ocultos.every((o) => o && ['manifiesto', 'reporte'].includes(o.tipo) && fecha(o.fecha))));
  ...
  // Un informe cuyo objeto quedó OCULTO en B2. Verificado contra B2 real el 2026-09-18: la credencial de escritura
  // puede ocultar (en B2 escribir incluye ocultar). No destruye nada —la versión retenida sobrevive—, pero quien
  // busque el informe sin pedir la versión recibe 404, así que la evidencia deja de estar a la vista.
  const ocultos = Array.isArray(estado.ocultos) ? estado.ocultos : [];
  if (ocultos.length) {
    abrir(db, 'informes_ocultos',
      `Hay ${ocultos.length} informe(s) firmados ocultos en Backblaze: el archivo sigue guardado, pero quien lo busque sin pedir la versión no lo ve. Alguien con la credencial de escritura lo ocultó; revisar quién.`,
      { ocultos: ocultos.slice(0, 10).map((o) => `${o.tipo}:${o.fecha}`) });
    return { estado: 'ocultos', esperado };
  }
  confirmarCicloSano(db, { integracion: INTEGRACION, proceso: PROCESO, tipoError: 'informes_ocultos' });
