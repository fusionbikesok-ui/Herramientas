    ...
    .then(resultado => {
      if (resultado?.enviado) {
        db.prepare("UPDATE incidentes_email_outbox SET estado='enviado', enviado_en=?, actualizado_en=? WHERE id=?")
          .run(now(), now(), fila.id);
        db.prepare(`INSERT INTO incidentes_operativos_historial (incidente_id, evento, detalle_json, creado_en)
          SELECT ?, ?, ?, ? WHERE NOT EXISTS (
            SELECT 1 FROM incidentes_operativos_historial WHERE incidente_id=? AND evento=?
          )`).run(incidente.id, fila.tipo === 'caida' ? 'email_alerta_caida' : 'email_alerta_recuperada',
          JSON.stringify({ destinatarios: process.env.ALERTAS_EMAIL ? 'configurados' : 'no_configurados' }), now(),
          incidente.id, fila.tipo === 'caida' ? 'email_alerta_caida' : 'email_alerta_recuperada');
      } else {
        const agotado = fila.intentos >= 8 ? 1 : 0;
        db.prepare("UPDATE incidentes_email_outbox SET estado='fallido', dlq=?, ultimo_error=?, actualizado_en=? WHERE id=?")
          .run(agotado, agotado ? 'DLQ: máximo de intentos alcanzado' : 'transporte SMTP no disponible', now(), fila.id);
      }
    })
    .catch(error => {
      try {
        const agotado = fila.intentos >= 8 ? 1 : 0;
        db.prepare("UPDATE incidentes_email_outbox SET estado='fallido', dlq=?, ultimo_error=?, actualizado_en=? WHERE id=?")
          .run(agotado, `${agotado ? 'DLQ: ' : ''}${String(error.message || 'error SMTP').slice(0, 480)}`, now(), fila.id);
      } catch (_) { /* fail-open */ }
    })
