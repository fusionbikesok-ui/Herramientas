 ...
 * Se detiene en el primer `platform_unavailable`: no tiene sentido martillar una plataforma caída.
 */
export async function importarPerdidas({ db, enviarSenal, limite = 100 }) {
  const filas = db.prepare(`SELECT event_id, channel, resource_id, metadata_json, shadow_reason, completed_at FROM integration_events
    WHERE shadow_status = 'discarded' AND shadow_reason IN ('platform_unavailable','platform_timeout') AND shadow_imported_at IS NULL
