 ...
 * Se detiene en el primer `platform_unavailable` o `cuenta_no_configurada`: no tiene sentido martillar una
 * plataforma caída, ni reintentar una cuenta que sigue sin configurar.
 */
export async function importarPerdidas({ db, enviarSenal, limite = 100 }) {
  ...
  const resultado = { importadas: 0, invalidas: 0, pendientes: filas.length, detenida: false };
  for (const f of filas) {
    ...
    try {
      ...
      // `cuenta_no_configurada` se detiene igual que una plataforma caída: si la cuenta sigue sin configurar,
      // reintentar los 347 recibos de la tanda en cada vuelta no arregla nada y llena el log.
