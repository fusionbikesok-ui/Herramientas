...
export const OPERACIONES = Object.freeze({
  ...
  'ml.items.multiget': (p) => {
    ...
    return { canal: 'ml', ruta: `/items?ids=${p.ids.join(',')}` };
  },
  'woo.orders.list': (p) => {
 ...
 */
export function crearGatewayCanal({ mlUserId, ejecutarMl, ejecutarWoo, presupuestoMl = crearPresupuestoShadow(0), metricas = null }) {
  let wooEnCurso = Promise.resolve();
  return async function ejecutar(peticion) {
    const op = construirOperacion(peticion, { mlUserId });
