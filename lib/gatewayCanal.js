...
/**
 * E1 T5 — fuente única del mapeo tópico de `missed_feeds` → corriente (spec §2.1). `TOPICOS_MISSED` y la
 * validación del transporte de la plataforma se derivan de esta tabla; no se mantienen listas separadas.
 */
...
const TOPICOS_MISSED = Object.freeze(Object.keys(TOPIC_A_CORRIENTE));

/** Mapeo operación → corriente para todo lo que no es `ml.missed_feeds` (spec §2.1). */
const OP_A_CORRIENTE = Object.freeze({
  'ml.orders.search': 'orders', 'ml.order': 'orders',
  'ml.shipment': 'shipments',
  'ml.items.scan': 'items', 'ml.items.multiget': 'items',
  'ml.questions.search': 'questions', 'ml.question': 'questions',
  'ml.messages.unread': 'messages', 'ml.messages.pack': 'messages',
  'ml.claims.search': 'claims', 'ml.claim': 'claims',
});

export const CORRIENTES_ML = Object.freeze([...new Set(Object.values(OP_A_CORRIENTE))]);

/** Consumidores del gateway (decisión de José, spec §2.8 opción b): E2/E3 no comparten cupo con E1. */
export const CONSUMIDORES = Object.freeze(['e1', 'catalogo', 'identidad']);

/**
 * Resuelve la corriente ML de una operación. `ml.missed_feeds` se deriva de `params.topic` en tiempo de
 * llamada; el resto sale de la tabla estática. Una operación ML sin corriente asignable es un error de
 * programación, no un caso de negocio.
 */
export function corrienteDe(op, params) {
  if (op === 'ml.missed_feeds') {
    const corriente = TOPIC_A_CORRIENTE[params?.topic];
    if (!corriente) throw new Error(`ml.missed_feeds: topic sin corriente asignada: ${params?.topic}`);
    return corriente;
  }
  const corriente = OP_A_CORRIENTE[op];
  if (!corriente) throw new Error(`operación ML sin corriente asignada: ${op}`);
  return corriente;
}
...
export function construirOperacion(peticion, ctx) {
  ...
  for (const k of Object.keys(peticion)) exigir(k === 'op' || k === 'params' || k === 'consumidor', k);
  if (peticion.consumidor !== undefined) exigir(CONSUMIDORES.includes(peticion.consumidor), 'consumidor');
...
const HEADERS_DEVUELTOS = ['x-wp-totalpages', 'x-wp-total', 'retry-after', 'x-fusion-cupo'];
const MAX_CUERPO_BYTES = 10 * 1024 * 1024;

const segundosHastaProximoMinuto = (ms) => Math.max(1, Math.ceil((60_000 - (ms % 60_000)) / 1000));

/**
 * Bucket simple por minuto: hasta `rpm` reservas por ventana. `rpm <= 0` cierra el bucket siempre
 * (fail-closed por defecto, spec §2.1).
 */
function crearBucket(rpm, ahora) {
  let ventana = -1; let usados = 0;
  return {
    intentar(ms) {
    ...
    segundosParaRefill(ms) { return segundosHastaProximoMinuto(ms); },
  };
}

/**
 * Presupuesto shadow de ML por corriente (E1 T5, spec §2.1/§2.2): cada corriente ML tiene su propio bucket
 * por minuto y todas comparten un techo global por encima. `configuracionPorCorriente` es
 * `{ orders, shipments, items, questions, messages, claims, e2e3 }` en rpm; `e2e3` es el bucket único y
 * separado de `catalogo`/`identidad` (decisión de José, spec §2.8 opción b). Devuelve un objeto con
 * `reservar(op, params, consumidor, ahora)` — la forma vieja `crearPresupuestoShadow(rpm)` (un solo bucket,
 * sin objeto) se sigue aceptando por compatibilidad con integraciones que no distinguen corriente.
 */
export function crearPresupuestoShadow(configuracionPorCorriente, rpmGlobal, ahora = () => Date.now()) {
  // Compatibilidad: firma vieja de un solo argumento numérico = un único bucket sin corrientes.
  if (typeof configuracionPorCorriente === 'number') {
    const rpmUnico = configuracionPorCorriente;
    const ahoraUnica = typeof rpmGlobal === 'function' ? rpmGlobal : ahora;
    const bucket = crearBucket(rpmUnico, ahoraUnica);
    return () => bucket.intentar(ahoraUnica());
  }
  const cfg = configuracionPorCorriente ?? {};
  const buckets = new Map();
  for (const corriente of CORRIENTES_ML) buckets.set(corriente, crearBucket(Number(cfg[corriente] || 0), ahora));
  buckets.set('e2e3', crearBucket(Number(cfg.e2e3 || 0), ahora));
  const global = crearBucket(Number(rpmGlobal || 0), ahora);
  return {
    reservar(op, params, consumidor = 'e1', ahoraArg) {
      const ms = ahoraArg ?? ahora();
      const corriente = consumidor === 'e1' ? corrienteDe(op, params) : 'e2e3';
      const bucket = buckets.get(corriente);
      // Orden: corriente primero, para no descontar el global si la corriente ya está cerrada.
      if (!bucket.intentar(ms)) return { ok: false, corriente, segundosParaRefill: bucket.segundosParaRefill(ms) };
      if (!global.intentar(ms)) return { ok: false, corriente: 'global', segundosParaRefill: global.segundosParaRefill(ms) };
      return { ok: true };
    },
  };
}
...
/**
 * Validación de arranque (spec §2.1): si la sombra está habilitada (`rpmGlobal > 0`) exige una variable de
 * cupo válida (entero > 0) para cada corriente ML y falla con un error explícito que nombra la faltante.
 * Con la sombra apagada no exige nada: el default de hoy sigue siendo cerrado sin configuración.
 */
export function validarConfiguracionCupoSombra(configuracionPorCorriente, rpmGlobal) {
  if (!(Number(rpmGlobal) > 0)) return;
  const cfg = configuracionPorCorriente ?? {};
  for (const corriente of CORRIENTES_ML) {
    const v = Number(cfg[corriente]);
    if (!(Number.isInteger(v) && v > 0)) {
      throw new Error(`GATEWAY_ML_SHADOW_RPM_${corriente.toUpperCase()} inválida o ausente con la sombra habilitada`);
    }
  }
  // e2e3 es opcional (default 0 = cerrado, spec §2.8): ausente no es error. Pero si está presente con un
  // valor que no es un entero >= 0 (typo), no debe colar silenciosamente como "cerrado" sin dejar rastro:
  // eso hace perder horas de diagnóstico si catálogo/identidad dejan de sincronizar.
  if (cfg.e2e3 !== undefined && cfg.e2e3 !== '') {
    const v = Number(cfg.e2e3);
    if (!(Number.isInteger(v) && v >= 0)) {
      throw new Error('GATEWAY_ML_SHADOW_RPM_E2E3 inválida (debe ser un entero >= 0) con la sombra habilitada');
    }
  }
}
...
export function crearGatewayCanal({ mlUserId, mlAppId = null, mlSiteId = null, ejecutarMl, ejecutarWoo, presupuestoMl = crearPresupuestoShadow(0), metricas = null }) {
  ...
  return async function ejecutar(peticion) {
    ...
    const consumidor = peticion.consumidor ?? 'e1';
    const inicio = Date.now();
    let res;
    let corrienteMetrica = null;
    if (op.canal === 'ml') {
      if (!mlUserId) throw new ErrorOperacionInvalida('ML no configurado en el legado');
      // `presupuestoMl` puede ser la forma vieja (función sin argumentos) o el objeto nuevo con `.reservar`.
      const decision = typeof presupuestoMl === 'function'
        ? (presupuestoMl() ? { ok: true } : { ok: false, corriente: null, segundosParaRefill: 60 })
        : presupuestoMl.reservar(peticion.op, peticion.params ?? {}, consumidor, Date.now());
      corrienteMetrica = decision.corriente ?? null;
      if (!decision.ok) {
        res = { status: 429, headers: { 'retry-after': String(decision.segundosParaRefill ?? 60), 'x-fusion-cupo': 'sombra-agotado' }, body: null };
      } else {
    ...
    metricas?.({ op: peticion.op, consumidor, corriente: corrienteMetrica, status: res.status, ms: Date.now() - inicio, permitida: res.status !== 429 || res.headers?.['x-fusion-cupo'] !== 'sombra-agotado' });
