...
export async function procesarCambios(db, mlCfg, cambios, opts = {}) {
  ...
  for (const c of lista) {
    ...
    if (clase === 'real' && c.campo === 'catalog_product_id' && c.valor_anterior && c.valor_nuevo) {
      ...
      clase = repetidos >= 1 || pareja?.size >= 2 ? 'migracion' : clase;
      if (clase !== 'migracion') {
        const leer = async (id) => {
          if (migracionCache.has(id)) return migracionCache.get(id);
          if (migracionCache.size >= MAX_LECTURAS_PRODUCTOS) return null;
          let r;
          try { r = await mlFetch(db, mlCfg, 'get', `/products/${id}`); } catch (e) { r = { error: e }; }
          migracionCache.set(id, r);
          return r;
        };
        ...
        const pairFresh = viejo?.status === 200 && nuevo?.status === 200 && Number.isFinite(fecha(viejo)) && Number.isFinite(fecha(nuevo))
          && ahora - fecha(viejo) <= 72 * 3600e3 && ahora - fecha(nuevo) <= 72 * 3600e3 && Math.abs(fecha(viejo) - fecha(nuevo)) <= 5 * 60e3;
        if (viejo?.status === 404 || pairFresh) clase = 'migracion';
      }
    }
    clases.push(clase);
  }
  ...
  if (!excede) {
    ...
    const estadoEnCache = db.prepare("SELECT status, sub_status, available_quantity FROM ml_publicaciones_cache WHERE item_id=? LIMIT 1");
    for (const itemId of items) {
      const idsDelItem = todos.map((t, i) => t.real && lista[i].item_id === itemId ? t.id : null).filter(Boolean);
      const cache = estadoEnCache.get(itemId);
      const sub = Array.isArray(cache?.sub_status) ? cache.sub_status.join(',') : String(cache?.sub_status || '');
      if (cache && ((cache.status === 'paused' && sub.includes('out_of_stock')) || cache.available_quantity === 0)) {
        sinStock += idsDelItem.length;
        db.prepare("UPDATE ml_publicacion_cambios SET revisado_en=?, revisado_por='vigia-auto', bloquea_reactivador=1, pausa_error=? WHERE id=?")
          .run(ts, 'sin stock en ML: se registra y queda bloqueado hasta revisión humana', idsDelItem[0]);
        for (const id of idsDelItem.slice(1)) db.prepare("UPDATE ml_publicacion_cambios SET revisado_en=?, revisado_por='vigia-auto', bloquea_reactivador=1, pausa_error=? WHERE id=?").run(ts, 'sin stock en ML: se registra y queda bloqueado hasta revisión humana', id);
        continue;
      }
      let error = null;
  ...
  abrirOActualizarIncidente(db, {
    ...
    mensajeHumano: excede
      ...
      : `${items.length} publicación(es) cambiaron de formato y se pausaron. ${detalle} Migraciones no pausadas: ${migracionesSinPausa}.`,
