...
export async function procesarCambios(db, mlCfg, cambios, opts = {}) {
    ...
    if (clase === 'real' && c.campo === 'catalog_product_id' && c.valor_anterior && c.valor_nuevo) {
      ...
      if (clase !== 'migracion') {
        ...
        const pairFresh = viejo?.status === 200 && nuevo?.status === 200 && Number.isFinite(fecha(viejo)) && Number.isFinite(fecha(nuevo))
          && ahora - fecha(viejo) >= -60e3 && ahora - fecha(viejo) <= 72 * 3600e3 && ahora - fecha(nuevo) >= -60e3 && ahora - fecha(nuevo) <= 72 * 3600e3 && Math.abs(fecha(viejo) - fecha(nuevo)) <= 5 * 60e3;
  ...
  const itemsSinStock = todosItems.filter(sinStockItem);
  let sinStock = 0;
    ...
    db.transaction(() => {
      for (const itemId of itemsSinStock) {
        for (const [i, t] of todos.entries()) {
          if (t.real && lista[i].item_id === itemId) { cerrar.run(ts, 'sin stock en ML: se registra y queda bloqueado hasta revisión humana', t.id); sinStock += 1; }
        }
      }
    })();
  }
