 ...
 *  - Solo si NO hay ya una decisión activa para esa clave (nunca pisa una decisión humana,
 *    ni siquiera una vieja — si ya se decidió algo, se respeta).
 ...
 *  - Solo si ese SKU no está YA vinculado a otra clave activa (un SKU-a-múltiples-publicaciones
 *    sin revisar es exactamente el tipo de ambigüedad que el Matcher existe para resolver).
 *
 * Se llama tanto después de cada refresco manual de publicaciones ML (routes/matcher.js)
 * como en el cron periódico de reconciliación (server.js, cada 10 min) — no depende de que
 * nadie abra el Matcher. Devuelve cuántas vinculó.
 */
export function autoVincularPorSellerSku(db) {
  const candidatos = db.prepare(`
    ...
    WHERE p.seller_sku IS NOT NULL AND p.seller_sku <> ''
      AND p.clave NOT IN (SELECT clave FROM sku_matcher_decisiones WHERE accion IN ('asignar','confirmar'))
  `).all();
