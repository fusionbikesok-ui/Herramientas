...
export function autoVincularPorSellerSku(db) {
  ...
  const tx = db.transaction(() => {
    for (const c of candidatos) {
      ...
      // .changes: si el ON CONFLICT DO NOTHING actuó (otro escritor tomó la clave entre el
      // SELECT de candidatas y este INSERT — Matcher/Cobertura escriben por HTTP en paralelo),
      // no contarla como vinculada (hallazgo del revisor: un contador incondicional mentiría).
      if (insertar.run(c.clave, c.sku, nombre, now).changes) vinculadas++;
    }
  });
  tx();
  return vinculadas;
...
export function clavesNecesitanAtencion(db) {
  const rows = db.prepare(`
    ...
    WHERE p.status = 'active'
      ...
      AND NOT EXISTS (SELECT 1 FROM errores_descartados e WHERE e.clave = p.clave)
    ORDER BY clave
  `).all();
