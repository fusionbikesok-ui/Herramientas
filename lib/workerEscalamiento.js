...
function dispositivosPara(db, item, escalado) {
  const asignado = item.assigned_user_id
    ? db.prepare('SELECT token FROM device_tokens WHERE user_id = ? AND revocado_en IS NULL').all(item.assigned_user_id)
    : [];
  ...
  return db.prepare('SELECT token FROM device_tokens WHERE revocado_en IS NULL LIMIT 50').all();
}
...
function supervisoresConDispositivo(db) {
  try {
    return db.prepare(`SELECT DISTINCT d.token FROM device_tokens d
...
export async function procesarEscalamiento(db, opciones = {}) {
  ...
  for (const { item, escalado, severidad } of aNotificar) {
    ...
    for (const dispositivo of dispositivosPara(db, item, escalado)) {
      try {
        await enviar(dispositivo.token, { ...payload, idempotencia_key: clave });
      } catch (e) {
