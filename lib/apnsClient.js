...
export async function enviarApns(deviceToken, payload, { entorno = 'production', cfg, sesionFactory } = {}) {
  ...
  try {
    return await new Promise((resolve) => {
      // Se declara antes de pedir el stream: `http2.connect` es asíncrono y la sesión puede
      // emitir 'error' (DNS, TLS, ECONNREFUSED, GOAWAY de Apple) antes de que exista un
      // stream. Sin este listener, Node trata el error de la sesión como excepción no
      // capturada y tira abajo el proceso entero, no sólo este envío.
      let temporizador;
      sesion.on('error', (e) => {
      ...
      try {
        ...
        // Si la sesión ya está cerrada o destruida (carrera, o GOAWAY justo antes), `request`
        // lanza sincrónico. Sin este catch, la promesa rechaza en vez de resolver con
        // {ok:false,...}, rompiendo el contrato de retorno de `enviarApns`.
        resolve({ ok: false, status: 0, reason: 'ErrorDeRed', mensaje: e.message, reintentable: true });
        return;
      }
