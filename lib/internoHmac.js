...
/** Keyring del emisor: además de las claves necesita saber con cuál firmar. */
export function cargarKeyringInternoActivo(ruta, io = {}) {
  const claves = cargarKeyringInterno(ruta, io);
  const leer = io.leer ?? ((r) => fs.readFileSync(r, 'utf8'));
  const activeKeyId = JSON.parse(leer(ruta)).activeKeyId;
  if (typeof activeKeyId !== 'string' || !Object.hasOwn(claves, activeKeyId)) throw new Error('keyring interno sin clave activa válida');
