...
export function fechaEstimadaShipment(shipment) {
  ...
  for (const valor of candidatos) {
    ...
    const match = /^(\d{4})-(\d{2})-(\d{2})(?:T|$)/.exec(valor);
    if (!match) continue;
    const fecha = new Date(`${match[1]}-${match[2]}-${match[3]}T12:00:00Z`);
    if (fecha.getUTCFullYear() === Number(match[1]) && fecha.getUTCMonth() + 1 === Number(match[2]) && fecha.getUTCDate() === Number(match[3])) {
