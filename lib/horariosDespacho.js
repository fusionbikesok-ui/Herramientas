...
function limiteCalendario(rows, fallback, ahora) {
  const horarios = normalizarHorarios(rows);
  const actual = partesBuenosAires(ahora);
  for (let offset = 0; offset < 8; offset += 1) {
    const dia = ((actual.weekday - 1 + offset) % 7) + 1;
    ...
    return { limite, fecha_local: fecha, estado: 'diferido', razon: 'CORTE_OPERATIVO_SUPERADO' };
  }
...
export function calcularSlaPreparacion({ canal, logisticType, shipment, horarios = [], ahora = new Date() }) {
  const actual = partesBuenosAires(ahora);
  if (canal === 'web') return { ...limiteCalendario(horarios, '15:00', ahora), fuente: 'web_horario_configurado' };
  ...
  if (logisticType === 'self_service') {
    return { ...limiteCalendario(horarios, '17:00', ahora), fuente: 'flex_horario_configurado' };
  }
