...
function limiteCalendario(rows, fallback, razonCorte, ahora) {
  const fuente = Array.isArray(rows) && rows.length ? rows : DIAS_SEMANA.map((dia) => ({ dia, habilitado: dia <= 5, hora_corte: fallback }));
  const horarios = normalizarHorarios(fuente);
  const actual = partesBuenosAires(ahora);
  for (let offset = 0; offset < 8; offset += 1) {
    const dia = ((actual.weekday - 1 + offset) % 7) + 1;
    const horario = horarios.find((h) => h.dia === dia);
    if (!horario?.habilitado) continue;
    const hora = horario.hora_corte || fallback;
    const fecha = sumarDias(actual.fecha, offset);
    const limite = localIsoConHora(fecha, hora);
    if (offset > 0 || actual.hora < hora) return { limite, fecha_local: fecha, estado: 'activo', razon: null };
    return { limite, fecha_local: fecha, estado: 'diferido', razon: razonCorte };
  }
  return { limite: null, fecha_local: null, estado: 'diferido', razon: 'SIN_DIA_OPERATIVO' };
}

export function calcularSlaPreparacion({ canal, logisticType, shipment, horarios = [], ahora = new Date() }) {
  const actual = partesBuenosAires(ahora);
  if (canal === 'web') return { ...limiteCalendario(horarios, '15:00', 'WEB_CORTE_15:00_SUPERADO', ahora), fuente: 'web_horario_configurado' };
  ...
  if (logisticType === 'self_service') {
    return { ...limiteCalendario(horarios, '17:00', 'FLEX_SALIDA_17:00_SUPERADA', ahora), fuente: 'flex_horario_configurado' };
  }
