 ...
 * @property {?number} precioLista     regular_price de Woo (precio de LISTA, sin oferta).
 *   `precio` es el VIGENTE (puede ser sale_price si el producto está en oferta) — para el
 *   contado de una venta ML nunca hay que usar `precio` a ciegas, ver precioContado().
 * @property {string} marca            Marca (taxonomía brands de WC); '' si no tiene
 * @property {string} gtin             Código universal (global_unique_id de WC); '' si no tiene
 */
...
export function normalizarProductoWc(raw) {
  ...
  // regular_price es el precio de LISTA (sin oferta); price es el VIGENTE, que si el
  // producto está en oferta ya es el sale_price. Se persisten los dos por separado: el
  // contado de una venta ML (precioContado()) tiene que aplicarse siempre sobre LISTA, o
  // una oferta activa se "acumularía" con el descuento de contado (decisión del usuario,
  // vigente; la historia detallada permanece en Git).
  const precioLista = raw.regular_price != null && raw.regular_price !== ''
    ? parseFloat(raw.regular_price) : null;
  return {
    id_woo: raw.id,
    nombre: raw.name,
    sku: raw.sku || '',
    ...
    categorias,
    // Un producto `simple` o `variable` también declara atributos en Woo, y ahí vive el dato
    // que distingue una unidad de otra (largo, medida, material). Hasta 2026-09-04 se
    // descartaban siempre: 0 de 1478 simples y 0 de 576 padres tenían atributos guardados, y
    // la pantalla de identidad no tenía con qué diferenciar productos de nombre idéntico.
    // Forma distinta a la de una variación: el producto trae `options` (lista) y la variación
    // `option` (uno solo). Se normalizan a la misma forma `{name, option}`.
    atributos: (Array.isArray(raw.attributes) ? raw.attributes : [])
      .map((a) => ({
        name: a?.name || '',
        option: Array.isArray(a?.options) ? a.options.filter(Boolean).join(', ') : (a?.option || ''),
      }))
      .filter((a) => a.name && a.option),
    img,
    precio: Number.isFinite(precio) ? precio : null,
    precioLista: Number.isFinite(precioLista) ? precioLista : null,
    marca,
    // global_unique_id lo trae tanto el producto simple como cada variación (el suyo propio).
    gtin: raw.global_unique_id || '',
  };
}
...
export function productoDesdeFilaCatalogo(row) {
  return {
    ...
    precioLista: row.regular_price ?? null,
    marca: row.marca ?? '',
    gtin: row.gtin ?? '',
  };
}
