...
export async function setStockWcDelta(cfg, db, sku, cantidadContada, stockInicial) {
  ...
  //
  // FAIL-CLOSED: si el stock AUMENTÓ durante el conteo (stockLive > stockInicial),
  // significa que algo externo sumó stock (ej. una recepción de mercadería, o un error
  // de conteo inicial). No podemos aplicar el delta a ciegas — no sabemos si fue una
  // recepción real o un error de medición, y aplicarlo sin más puede sobre-publicar
  // stock que no existe. Por eso lo tratamos como un caso que necesita revisión humana:
  // lanzamos error explícito y dejamos que caiga en fallidos, igual que otros errores
  // de Woo (no es transitorio, es sistemático — un reintento sería idéntico).
  if (stockLive > stockInicial) {
    throw new Error(
      `Stock aumentó durante el conteo para SKU "${sku}" (inicial=${stockInicial}, ` +
      `live=${stockLive}, contada=${cantidadContada}). Requiere revisión manual: puede ` +
      `ser ingreso de mercadería no registrado o error de medición inicial.`
    );
  }
