...
// Atributos de catálogo que NO son dimensión de variante (marca, categoría/tipo, género,
// diseño, compuesto, material, etc.) — nunca deben clasificarse como color/talle aunque su
// valor no matchee ningún patrón conocido. Verificado contra catalogo_cache real (2026-09-23):
// los productos IGPSPORT del piloto Pedalar #205 solo tienen el atributo "Marca" estructurado,
// y el catch-all metía su valor ("igpsport") entero en `talles`, generando contradicciones
// fantasma contra cualquier documento que declarara o infiriera cualquier talle. Lista cerrada
// a propósito (José): Velocidades/Dientes/Largo/Ancho/Rodado/Altura SÍ son dimensiones de
// variante reales (11v/12v, 32t/34t) y deben seguir discriminando — no se toca el catch-all
// para esos, solo se excluyen por nombre los que no lo son.
const ATRIBUTOS_NO_VARIANTE = new Set([
  'marca', 'tipo de producto', 'tipo de articulo', 'tipo de montaje', 'genero', 'diseno',
  'compuesto', 'body', 'material del cuadro', 'installment',
]);
...
// DEFECTO B3 (piloto Pedalar #205, José): normalización genérica liviana de género/número,
// SOLO para tokens alfabéticos de 6+ letras (evita falsos positivos en palabras cortas donde
// quitar una letra cambia el significado). Sin lista de pares a mano: "cardiaca"→"cardiac",
// "cardiaco"→"cardiac", "ciclocomputadora"→"ciclocomputador" (ya termina en consonante, no se
// toca), "ciclocomputadores"→"ciclocomputador". No es un stemmer completo: solo el sufijo -a/-o
// final (género) y el plural -es/-s, aplicados en ese orden.
export function normalizarGeneroNumero(tok) {
  if (!tok || tok.length < 6 || !/^[a-z]+$/.test(tok)) return tok;
  let t = tok;
  if (t.endsWith('es') && t.length > 7) t = t.slice(0, -2);
  else if (t.endsWith('s') && t.length > 6) t = t.slice(0, -1);
  if (t.endsWith('a') || t.endsWith('o')) t = t.slice(0, -1);
  return t;
}
export function canonToken(tok) { return colorCanonico(tok) || medidaCanonica(tok) || normalizarGeneroNumero(tok); }
