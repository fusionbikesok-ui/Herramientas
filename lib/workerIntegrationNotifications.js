...
export async function procesarEntregasPush(db, { send = enviarNotificacion, limit = 50 } = {}) {
  // Fail-closed: la ausencia de PUSH_REAL_ENABLED o cualquier valor distinto de 'true'
  // mantiene las entregas pausadas, sin depender de NODE_ENV. Solo `'true'` explícito habilita.
  const pushRealEnabled = process.env.PUSH_REAL_ENABLED === 'true';
  const ts = now();
  const rows = db.prepare(`
    SELECT d.*, n.title, n.body, n.deep_link, dt.token, COALESCE(i.priority, 'normal') AS priority
    ...
    LEFT JOIN inbox_items i ON i.inbox_id = n.inbox_id
    ...
    ORDER BY CASE COALESCE(i.priority, 'normal') WHEN 'urgent' THEN 0 WHEN 'high' THEN 1 ELSE 2 END,
      d.delivery_id LIMIT ?`).all(ts, ts, limit);
  let processed = 0;
  for (const row of rows) {
    if (!pushRealEnabled) continue;
