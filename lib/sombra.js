...
const RAZONES = Object.freeze([
  ...
  'platform_unavailable', 'invalid_resource', 'cuenta_no_configurada', 'process_stopped', 'response_not_finished',
  // Canario (C10): canal no habilitado o recurso fuera del porcentaje. No es un error: es la muestra.
  'canary_excluded',
]);
...
/**
 * Canario de la copia (C10, decisiones de José 2026-09-17): por canal y por porcentaje DETERMINÍSTICO por
 * recurso. El mismo pedido o producto siempre entra o siempre queda afuera, así un recurso nunca tiene
 * copias salteadas. SOMBRA_CANALES="woo" (o "woo,ml"); SOMBRA_PORCENTAJE=1..100. Sin variables: todo.
 */
export function crearSelectorCanario(env = process.env) {
  const canales = new Set(String(env.SOMBRA_CANALES || 'ml,woo').split(',').map((c) => c.trim()).filter(Boolean));
  const bruto = env.SOMBRA_PORCENTAJE === undefined || env.SOMBRA_PORCENTAJE === '' ? 100 : Number(env.SOMBRA_PORCENTAJE);
  const porcentaje = Number.isFinite(bruto) ? Math.min(100, Math.max(0, bruto)) : 0; // valor inválido = nada
  return {
    canales: [...canales], porcentaje,
    incluye(canal, recurso) {
      if (!canales.has(canal)) return false;
      if (porcentaje >= 100) return true;
      const h = crypto.createHash('sha256').update(`${canal}|${recurso}`).digest();
      return h.readUInt32BE(0) % 100 < porcentaje;
    },
  };
}
...
export function crearColaSombra({ db, enviar, capacidad = CAPACIDAD, concurrencia = CONCURRENCIA, timeoutMs = TIMEOUT_MS, alFinalizar = null }) {
  ...
  async function intentar(trabajo) {
    ...
    try {
      ...
      const razon = ['platform_timeout', 'invalid_resource', 'cuenta_no_configurada'].includes(error?.message) ? error.message : 'platform_unavailable';
