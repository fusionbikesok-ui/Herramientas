...
// `intervalo_min` es NOMINAL. La cadencia real es un tick del cron más larga —15 corre cada
// 20, 20 cada 25, 30 cada 35, 60 cada 65— por la condición de borde documentada en `tocaScan`.
// Para razonar sobre frescura hay que usar la real, no la de esta tabla.
const ESCALONES = [
...
/**
 * La comparación es contra el intervalo EXACTO, y eso hace que la cadencia real
 * sea un tick del cron más larga que la nominal.
 *
 * El scan se registra unos milisegundos después del tick que lo lanzó (medido el
 * 2026-09-06: tick 15:30:00.0xx → `ultimo_scan_en` 15:30:00.160). Quince minutos
 * después faltan esos 160 ms para llegar al umbral, el tick de las 15:45 no
 * dispara, y corre recién a las 15:50. Como el nuevo timestamp vuelve a quedar
 * unos ms tarde, el desfase se repite indefinidamente: con el tick de cinco
 * minutos del cron, un
 * intervalo de 15 min corre cada 20, uno de 20 cada 25, y así.
 *
 * Se deja así a propósito (decisión del usuario, 2026-09-06). Darle una
 * tolerancia haría que el intervalo se cumpla, pero eso son 96 scans diarios en
 * vez de 72 —de 14.184 a 18.912 llamadas a ML por día, un 33% más— contra una
 * API que ya nos bloqueó una vez. La frescura efectiva vale más documentada que
 * corregida a ese precio.
 *
 * Cadencia REAL de cada escalón, que es la que hay que usar para razonar sobre
 * frescura: 15→20 min, 20→25, 30→35, 60→65.
 *
 * Si algún día se corrige, hay que bajar un escalón el ramp en la misma tanda
 * para no subir el consumo sin querer.
 */
