...
import { construirWC, construirML, candidatosParaWC, diffTokens, toks, tsr, confianzaDesdeScore, normalizarMarcas, otrasMarcasPosibles } from './matcherEngine.js';
import { inicioHoyBuenosAiresISO } from './tiempo.js';
...
export function progresoHoy(db) {
  const desde = inicioHoyBuenosAiresISO();
...
/**
 * "Seguir donde quedé": marca de la última sesión DE ESTE USUARIO, con su progreso actual.
 * Por usuario y por dirección (migración 012) — antes era un singleton compartido por todos
 * los usuarios; con el Matcher unificado (Joaco gana acceso), dos personas trabajando la cola
 * al mismo tiempo se pisaban el "seguir donde quedé" mutuamente. `direccion` queda fija en
 * 'wc_ml' en esta entrega (única dirección activa) — la entrega 2 (ML→WC) le pasará la suya.
 * Sin userId (nadie autenticado en el request, no debería pasar detrás de authGuard) no hay
 * sesión de quién retomar: devuelve null en vez de adivinar.
 */
export function seguirDondeQuede(db, userId, direccion = 'wc_ml') {
  if (userId == null) return null;
  const sesion = db.prepare(
    'SELECT marca_actual FROM cobertura_sesion WHERE user_id = ? AND direccion = ?'
  ).get(userId, direccion);
...
/**
 * Registra/actualiza la marca en trabajo de ESTE USUARIO (para "seguir donde quedé"). Sin
 * userId es un no-op: no hay sesión de quién a la que atarla (mismo criterio que
 * seguirDondeQuede).
 */
export function tocarSesion(db, marca, userId, direccion = 'wc_ml') {
  if (userId == null) return;
  db.prepare(`
    INSERT INTO cobertura_sesion (user_id, direccion, marca_actual, actualizado_en) VALUES (?, ?, ?, ?)
    ON CONFLICT(user_id, direccion) DO UPDATE SET marca_actual = excluded.marca_actual, actualizado_en = excluded.actualizado_en
  `).run(userId, direccion, marca, new Date().toISOString());
}
...
export function construirIndiceMlSinSku(db) {
  ...
  `).all();
  const mlIndex = construirML(rows.map((r) => ({
  ...
  // Lista de marcas conocidas del catálogo (para marcaEnConflicto en matcherEngine): el motor
  // es puro y no consulta la DB, así que se arma acá (única fuente: catalogo_cache.marca) y se
  // cuelga del índice. Se recalcula en cada llamada (igual que el resto del índice): el volumen
  // de marcas distintas es chico y esto no está en un loop caliente.
  const marcas = db.prepare(
    "SELECT DISTINCT marca FROM catalogo_cache WHERE marca IS NOT NULL AND marca != ''"
  ).all().map((r) => r.marca);
  mlIndex.marcasConocidas = normalizarMarcas(marcas);
  return mlIndex;
}
...
export function candidatosDeProducto(prod, mlIndex) {
  ...
  // marcasConocidas NO se reenvía a mano: candidatosParaWC la lee de mlIndex.marcasConocidas
  // (construirIndiceMlSinSku ya la cuelga ahí arriba). Un solo lugar donde puede faltar.
...
export function buscarMlManual(prod, mlIndex, query, limit = 20) {
  ...
  // Una vez por búsqueda, no una vez por resultado (ver otrasMarcasPosibles en matcherEngine.js).
  const otrasMarcas = otrasMarcasPosibles(prod.marca || '', mlIndex.marcasConocidas);
  return mlIndex.mlItems
    .filter((m) => m.titulo.toLowerCase().includes(q))
    .slice(0, limit)
    .map((m) => {
      const diff = diffTokens(tksWc, m.tokens, mlIndex.df, mlIndex.corpusSize, prod.marca || '', otrasMarcas);
