...
export function cambiarEstadoDevolucionProveedor(db, id, input = {}) {
  const op = requiredOperation(input.operation_id), version = expected(input.expected_version);
  const target = String(input.estado || '').trim();
  const allowed = { pendiente: ['preparada', 'cancelada'], preparada: ['enviada', 'cancelada'], enviada: ['recibida'], recibida: [], cancelada: [] };
  const usuario = actor(input.cambiado_por), motivo = String(input.motivo || '').trim();
  if (!Object.prototype.hasOwnProperty.call(allowed, target) || !motivo) return resultError('INVALID_INPUT');
  const duplicateEvent = db.prepare('SELECT * FROM stock_supplier_return_events WHERE operation_id=?').get(op);
  if (duplicateEvent) return { ok: true, repetido: true, devolucion: db.prepare('SELECT * FROM stock_supplier_returns WHERE id=?').get(Number(id)) };
  const ts = now();
  return db.transaction(() => {
    const before = db.prepare('SELECT * FROM stock_supplier_returns WHERE id=?').get(Number(id));
    if (!before) return resultError('NOT_FOUND');
    if (before.expected_version !== version) return resultError('VERSION_CONFLICT');
    if (!allowed[before.estado].includes(target)) return resultError('INVALID_TRANSITION');
    const changed = db.prepare('UPDATE stock_supplier_returns SET estado=?, expected_version=expected_version+1, cambiado_por=?, cambiado_en=?, actualizado_en=? WHERE id=? AND expected_version=?').run(target, usuario, ts, ts, Number(id), version);
    if (!changed.changes) return resultError('VERSION_CONFLICT');
    const after = db.prepare('SELECT * FROM stock_supplier_returns WHERE id=?').get(Number(id));
    db.prepare(`INSERT INTO stock_supplier_return_events (devolucion_id,evento,actor,antes_json,despues_json,motivo,operation_id,creado_en) VALUES (?,?,?,?,?,?,?,?)`).run(Number(id), `estado_${target}`, usuario, JSON.stringify(before), JSON.stringify(after), motivo, op, ts);
    return { ok: true, devolucion: after };
  })();
}

export function listarEventosDevolucionProveedor(db, id) {
  return db.prepare('SELECT * FROM stock_supplier_return_events WHERE devolucion_id=? ORDER BY id').all(Number(id));
}
...
export function crearSenderWooExcepciones(cfg, db) {
  return async (fila) => {
    ...
    const confirmado = await getStockLiveWc(cfg, db, fila.sku);
    if (Number(confirmado) !== stockFinal) {
      throw new Error(`Reconciliación Woo inconsistente para SKU "${fila.sku}": esperado=${stockFinal}, remoto=${confirmado}`);
    }
    return { sku: fila.sku, stockLive: Number(stockLive), stockFinal, confirmado: Number(confirmado), delta: Number(fila.delta) };
  };
}
