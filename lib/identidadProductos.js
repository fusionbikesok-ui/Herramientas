...
export function conciliacionIdentidad(db) {
  const universo = `SELECT clave FROM ml_publicaciones_cache
    WHERE status='active' AND COALESCE(available_quantity,0)>0`;
  ...
  const incompletas = db.prepare(`SELECT COUNT(*) n FROM ml_publicaciones_cache
    WHERE status='active' AND COALESCE(available_quantity,0)>0 AND atributos_json IS NULL`).get().n;
...
export function listarCasosIdentidad(db, { direccion = 'ml_fusion', estado, pendientes } = {}) {
  ...
  return db.prepare(`SELECT c.*,p.nombre_canonico,p.fusion_sku,m.titulo,m.seller_sku,m.seller_custom_field,
    m.gtin,m.available_quantity,m.thumbnail,m.permalink,m.variations_texto,m.actualizado_en AS ml_observado_en
    ...
    .all(...params).map((c) => ({ ...c, thumbnail: imagenSegura(c.thumbnail) }));
}
...
export function obtenerCasoIdentidad(db, id) {
  ...
  return { ...caso,
    publicacion: pub ? { ...pub, thumbnail: imagenSegura(pub.thumbnail) } : null,
    producto: producto ? { ...producto, img: imagenSegura(producto.img) } : null,
...
export function buscarProductosFusion(db, { q = '', limite = 20 } = {}) {
  ...
  for (const f of filas) {
    ...
    const todos = tokens.every((t) => pajar.includes(t));
    if (!identExacto && !identParcial && !todos) continue;
    ...
    puntuadas.push({ f, orden: identExacto ? 2 : identParcial ? 1 : 0, similitud, sobrantes });
  }
  puntuadas.sort((a, b) => b.orden - a.orden || b.similitud - a.similitud || a.sobrantes - b.sobrantes
    || String(a.f.nombre_canonico).localeCompare(String(b.f.nombre_canonico), 'es'));
  return puntuadas.slice(0, tope).map(({ f }) => ({ ...f, img: imagenSegura(f.img) }));
}
