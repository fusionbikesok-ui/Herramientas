...
export function protegerPorBajaWoo(db, { idWoo = null, sku = null, motivo = 'baja_woo', confirmado = false }, actor = 'sistema') {
  ...
  if (afectadas.length === 0) return { ok: true, claves: 0, casos: 0, pedidos_retenidos: 0 };
  ...
  db.transaction(() => {
    for (const a of afectadas) {
      ...
      casos += 1;
      registrarHistorial(db, 'caso', null, 'woo_baja_protegida', actor, {
  ...
  return { ok: true, claves: afectadas.length, casos, pedidos_retenidos: pedidos };
}
