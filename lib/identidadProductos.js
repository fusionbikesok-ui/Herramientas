...
export async function procesarOperacionesIdentidad(db, adapter, { ahora = new Date() } = {}) {
  ...
  const pendientes = db.prepare(`SELECT id FROM identidad_operaciones
    ...
    ORDER BY id LIMIT ?`).all(...(canario ? [canario, ahora.toISOString(), tope] : [ahora.toISOString(), tope]));

  const resultados = [];
  for (const { id } of pendientes) {
    try {
      resultados.push({ id, ...(await procesarPasoOperacionIdentidad(db, id, adapter, { allowRemoteWrites: habilitado })) });
    } catch (e) {
