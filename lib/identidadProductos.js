...
export function auditarIdentidadProductos(db, actor = 'sistema', { lecturaConfiable = false, ahora = new Date() } = {}) {
  ...
  const auditadas = publicaciones.length - incompletas;
  // El gate 2 no se da por cumplido con la igualdad sola: se cumplía igual de bien con el
  // 100% de la clasificación equivocada. Exige además que no quede ninguna observación
  // incompleta, o el universo no está realmente conciliado.
  return { total: publicaciones.length, auditadas, observacion_incompleta: incompletas,
    verificadas, excepciones: exceptuadas, urgentes,
    conciliado: incompletas === 0 && auditadas === verificadas + exceptuadas + urgentes,
    escaneado_en: ts };
}
