...
function upsertCaso(db, pub, clasificacion, productoId, fp, estado, ts) {
  ...
  const ultimaOperacion = db.prepare('SELECT estado,producto_id FROM identidad_operaciones WHERE caso_id=? ORDER BY id DESC LIMIT 1')
    .get(anterior.id);
  ...
  if (cambio) {
    if (identidadAplicada) {
      ...
      // Un caso `pendiente` ya tiene decisión tomada y operación encolada: devolverlo a la cola
      // como si nadie lo hubiera visto borra ese trabajo. Si la identidad cambió de verdad, va a
      // `intervencion`, que exige una persona y conserva la decisión.
  ...
  db.prepare(`UPDATE identidad_casos SET producto_id=?,clasificacion=?,estado=?,severidad=?,
    ...
    WHERE id=?`).run(productoId, clasificacion, proximoEstado, proximoEstado === 'verificado' ? 'normal' : 'urgente',
      fp, ts, proximoEstado === 'verificado' ? ts : null, cambio || recuperacionEstado ? 1 : 0, anterior.id);
...
export function auditarIdentidadProductos(db, actor = 'sistema', { lecturaConfiable = false, ahora = new Date() } = {}) {
  ...
  return db.transaction(() => {
    ...
    for (const pub of publicaciones) {
      ...
      const fp = `v2:${fingerprintEvidencia(evidencia)}`;
  ...
  // trabajo: se cierra como fuera de alcance en vez de quedar urgente para siempre.
  db.prepare(`UPDATE identidad_casos SET estado='resuelto',resuelto_en=?,ultima_deteccion_en=?
    WHERE direccion='ml_fusion' AND estado IN ('urgente','tomado')
      AND ml_key NOT IN (SELECT clave FROM ml_publicaciones_cache
        WHERE status='active' AND COALESCE(available_quantity,0)>0 AND ${FILTRO_MARKETPLACE})`).run(ts, ts);
 ...
 *  2. si hay `canario_ml_key`, SOLO se procesa esa clave — el resto espera;
...
export async function procesarOperacionesIdentidad(db, adapter, { ahora = new Date() } = {}) {
  ...
  const canario = String(cfg.canario_ml_key || '').trim();
  const tope = Math.max(1, Math.min(50, Number(cfg.lote_max) || 1));
  ...
  const pendientes = db.prepare(`SELECT id FROM identidad_operaciones
    ...
    ORDER BY id LIMIT ?`)
    .all(...(canario ? [canario, ahora.toISOString(), ahora.toISOString(), tope]
  ...
  return { ok: true, canario: canario || null, tope, procesadas: resultados.length, resultados };
}
