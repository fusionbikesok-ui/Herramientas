...
export function buscarProductosFusion(db, { q = '', limite = 20, gtin_ml: gtinMl, sku_ml: skuMl } = {}) {
  ...
  for (const f of filas) {
    ...
    const orden = coincideGtin ? 4 : coincideSku ? 3 : identExacto ? 2 : identParcial ? 1 : 0;
    puntuadas.push({ f, orden, similitud, sobrantes, coincideGtin, coincideSku, todos, presentes });
  }
  ...
  const completo = (x) => x.todos || x.orden >= 1;
  ...
  return elegidas.map(({ f, coincideGtin, coincideSku, todos, orden }) => ({
    ...f,
    ...
    coincidencia_parcial: !(todos || orden >= 1),
  }));
}
