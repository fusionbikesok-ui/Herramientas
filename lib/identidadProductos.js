...
import { frescuraVigenteMs } from './mlScanRamp.js';

// Frescura por defecto. La vigente la manda el ramp del scan (`lib/mlScanRamp.js`): si el
// intervalo del scan sube y la frescura no, justo antes de cada corrida toda observación
// queda vieja y NADA verifica. Los dos valores se mueven juntos, nunca por separado.
const FRESCURA_MS = 60 * 60 * 1000;
function frescuraMs(db) {
  try { return frescuraVigenteMs(db); } catch { return FRESCURA_MS; }
}
...
// Una protección por baja de Woo termina en el cero verificado: no hay SKU que escribir ni
// stock que restaurar, porque el producto ya no existe en Woo. Reusa el mismo ejecutor —y por
// lo tanto los mismos frenos: modo, canario, lote, claim, reintentos— con su propia secuencia.
const PASOS_PROTECCION = ['zero', 'verify_zero'];
const pasosDe = (op) => (op?.tipo === 'proteccion_woo' ? PASOS_PROTECCION : PASOS);
...
// `normalizarGtin` se llamaba así pero no normalizaba: validaba el largo y
// devolvía el código crudo. Por eso 0602883701731 y 602883701731 se trataban
// como códigos distintos. La regla real vive en lib/gtin.js; acá se conserva
// `esGtinValido` con su nombre histórico porque describe bien lo que hace.
import { esGtinCrudoValido, claveGtin, normalizarGtin as analizarGtin } from './gtin.js';

export { esGtinCrudoValido as esGtinValido, claveGtin };
...
export function bootstrapProductosFusion(db, actor = 'sistema') {
  ...
  const prioridadWoo = String(db.prepare('SELECT identificadores_prioridad p FROM identidad_config WHERE id=1').get()?.p || 'woo,ml,manual')
    .split(',').map((x) => x.trim());
  const ordenWoo = (prioridadWoo.indexOf('woo') + 1 || prioridadWoo.length + 1) * 10;
  db.transaction(() => {
    ...
    for (const woo of productos) {
      ...
      const codigoWoo = analizarGtin(woo.gtin);
      for (const [tipo, valor] of [['fusion_sku', producto.fusion_sku], ['woo_sku', woo.sku], ['gtin', claveGtin(woo.gtin)]]) {
        if (!valor) continue;
        // El GTIN de Woo guarda además su representación original, su subtipo y
        // el orden inicial de la fuente `woo`; los SKU son su propia clave.
        db.prepare(`INSERT OR IGNORE INTO identificadores_producto
          (tipo,valor_normalizado,valor_crudo,subtipo,fuente,producto_id,estado,orden,creado_en,actualizado_en)
          VALUES (?,?,?,?,?,?,'activo',?,?,?)`)
          .run(tipo, valor,
            tipo === 'gtin' ? codigoWoo.crudo : null,
            tipo === 'gtin' ? codigoWoo.tipo : null,
            tipo === 'gtin' ? 'woo' : null,
            producto.id, tipo === 'gtin' ? ordenWoo : 100, ts, ts);
      }
    }
  }).immediate();
...
/**
 * Identificador que representa al producto: el primero de la lista ordenada,
 * como el orden de arranque de una BIOS.
 *
 * No hay una regla fija de "la fuente X siempre gana": un producto puede llevar
 * varios códigos legítimos —los que el mismo artículo tiene en distintos
 * mercados— y cuál lo representa es una decisión del negocio, reordenable desde
 * Productos Fusion. `identidad_config.identificadores_prioridad` sólo fija el
 * orden inicial de los identificadores nuevos.
 *
 * El desempate por `id` mantiene el resultado estable cuando dos comparten
 * `orden`, que es lo normal después de un backfill: sin él, la proyección a Woo
 * podría cambiar sola entre dos corridas sin que nadie tocara nada.
 */
export function identificadorPrincipal(db, productoId, tipo = 'gtin') {
  return db.prepare(`SELECT * FROM identificadores_producto
    WHERE producto_id=? AND tipo=? AND estado='activo'
    ORDER BY orden, id LIMIT 1`).get(productoId, tipo) || null;
}

/**
 * Reordena los identificadores activos de un producto. `valoresEnOrden` es la
 * lista de `valor_normalizado` en la prioridad deseada; el primero gana.
 *
 * Se rechaza una lista incompleta en vez de completarla sola: si el llamador
 * cree estar ordenando cuatro códigos y el producto tiene cinco, el quinto
 * quedaría en una posición que nadie eligió, y podría terminar primero.
 */
export function reordenarIdentificadores(db, productoId, valoresEnOrden, tipo = 'gtin') {
  const actuales = db.prepare(`SELECT id, valor_normalizado FROM identificadores_producto
    WHERE producto_id=? AND tipo=? AND estado='activo'`).all(productoId, tipo);
  const pedidos = valoresEnOrden.map(String);
  if (pedidos.length !== actuales.length || new Set(pedidos).size !== pedidos.length) {
    return { ok: false, error: 'la lista debe traer exactamente una vez cada identificador activo' };
  }
  const porValor = new Map(actuales.map((r) => [r.valor_normalizado, r.id]));
  if (pedidos.some((v) => !porValor.has(v))) {
    return { ok: false, error: 'la lista incluye un identificador que no está activo en este producto' };
  }
  const ts = now();
  const actualizar = db.prepare('UPDATE identificadores_producto SET orden=?, actualizado_en=? WHERE id=?');
  db.transaction(() => {
    pedidos.forEach((valor, i) => actualizar.run((i + 1) * 10, ts, porValor.get(valor)));
  })();
  return { ok: true, principal: pedidos[0] };
}

/**
 * Siembra como identificadores los GTIN que sólo conocía ML.
 *
 * Hasta ahora `identificadores_producto` se llenaba únicamente desde Woo: 740
 * filas `gtin` para 5141 productos, exactamente los GTIN válidos del catálogo.
 * Los 5241 GTIN válidos que ML publica vivían sólo en `ml_publicaciones_cache`
 * y no participaban de la identidad.
 *
 * Al incorporarlos aparecen colisiones que antes no existían porque había una
 * sola fuente. Se tratan en tres grupos, medidos sobre producción el
 * 2026-09-06 (98 canónicos compartidos por más de un SKU):
 *
 *   - 28 son variaciones del mismo producto padre Woo, que heredaron el GTIN
 *     del padre. Es deuda de catálogo, no ambigüedad de identidad: no confunden
 *     dos productos distintos, así que se absorben sin generar trabajo humano.
 *   - 22 enfrentan productos de padres Woo distintos. Son conflictos reales:
 *     se registran en estado `conflicto` para que quede la evidencia y los
 *     decida una persona. No se auto-resuelven, y no se descartan.
 *   - 48 involucran un `seller_sku` que no existe en Woo. No son un problema de
 *     identificadores: se devuelven aparte para el detector de publicaciones
 *     sin respaldo, marcando las que están activas y con stock.
 *
 * Idempotente: se puede correr en cada arranque sin duplicar ni reabrir nada.
 */
export function sembrarIdentificadoresMl(db, actor = 'sistema') {
  const ts = now();
  const publicaciones = db.prepare(`SELECT clave,seller_sku,gtin,available_quantity,status
    FROM ml_publicaciones_cache
    WHERE gtin IS NOT NULL AND TRIM(gtin) <> '' AND seller_sku IS NOT NULL AND TRIM(seller_sku) <> ''
    ORDER BY clave`).all();

  const wooPorSku = db.prepare('SELECT id_woo,id_padre FROM catalogo_cache WHERE sku=? ORDER BY id_woo');
  const productoPorWoo = db.prepare('SELECT id FROM productos_fusion WHERE primary_woo_id=?');
  const padrePorProducto = db.prepare(`SELECT COALESCE(c.id_padre, c.id_woo) AS raiz
    FROM productos_fusion p JOIN catalogo_cache c ON c.id_woo = p.primary_woo_id WHERE p.id=?`);
  const activoPorValor = db.prepare(`SELECT id,producto_id FROM identificadores_producto
    WHERE tipo='gtin' AND valor_normalizado=? AND estado='activo'`);
  const yaRegistrado = db.prepare(`SELECT 1 FROM identificadores_producto
    WHERE tipo='gtin' AND valor_normalizado=? AND producto_id=?`);
  const insertar = db.prepare(`INSERT INTO identificadores_producto
    (tipo,valor_normalizado,valor_crudo,subtipo,fuente,producto_id,estado,orden,creado_en,actualizado_en)
    VALUES ('gtin',?,?,?,'ml',?,?,?,?,?)`);
  // El orden inicial sale de la prioridad configurada, no de una regla fija en
  // el código: es una decisión de negocio y se reordena desde Productos Fusion.
  const prioridad = String(db.prepare('SELECT identificadores_prioridad p FROM identidad_config WHERE id=1').get()?.p || 'woo,ml,manual')
    .split(',').map((x) => x.trim());
  const ordenMl = (prioridad.indexOf('ml') + 1 || prioridad.length + 1) * 10;

  const resultado = { sembrados: 0, conflictos: 0, absorbidos: 0, sinRespaldoWoo: [] };

  // Un solo recorrido transaccional por el mismo motivo que el bootstrap: con
  // journal_mode=delete, miles de statements sueltos son minutos de fsync.
  db.transaction(() => {
    for (const pub of publicaciones) {
      const codigo = analizarGtin(pub.gtin);
      if (!codigo.ok) continue;
      const filas = wooPorSku.all(String(pub.seller_sku).trim());
      if (filas.length === 0) {
        // Sin respaldo en Woo: no es un identificador que podamos anclar a nada.
        resultado.sinRespaldoWoo.push({
          clave: pub.clave,
          seller_sku: pub.seller_sku,
          gtin: pub.gtin,
          stock: Number(pub.available_quantity) || 0,
          activa: String(pub.status || '') === 'active',
        });
        continue;
      }
      // Un SKU en más de un producto Woo ya lo reporta `sku_no_unico`; sembrar
      // un identificador acá elegiría un producto arbitrario.
      if (filas.length > 1) continue;
      const producto = productoPorWoo.get(filas[0].id_woo);
      if (!producto) continue;
      if (yaRegistrado.get(codigo.canonico, producto.id)) continue;

      const activo = activoPorValor.get(codigo.canonico);
      if (activo && activo.producto_id !== producto.id) {
        const raizExistente = padrePorProducto.get(activo.producto_id)?.raiz;
        const raizNueva = padrePorProducto.get(producto.id)?.raiz;
        if (raizExistente != null && raizExistente === raizNueva) {
          // Variaciones del mismo padre: el identificador que ya está activo
          // cubre al producto, no hace falta una fila más ni un caso.
          resultado.absorbidos += 1;
          continue;
        }
        insertar.run(codigo.canonico, codigo.crudo, codigo.tipo, producto.id, 'conflicto', ordenMl, ts, ts);
        registrarHistorial(db, 'identificador', producto.id, 'gtin_en_conflicto', actor, {
          gtin: codigo.canonico, ml_key: pub.clave, producto_en_conflicto: activo.producto_id,
          stock_ml: Number(pub.available_quantity) || 0,
        }, ts);
        resultado.conflictos += 1;
        continue;
      }
      if (activo) continue;
      insertar.run(codigo.canonico, codigo.crudo, codigo.tipo, producto.id, 'activo', ordenMl, ts, ts);
      resultado.sembrados += 1;
    }
  }).immediate();
...
function lookupWoo(db, sellerSku, gtin) {
  ...
  const gtinCanonico = claveGtin(gtin);
  // catalogo_cache.gtin guarda el código crudo de Woo, así que se rellena en la
  // consulta con la misma expresión de texto que usa migrations/088: comparar
  // crudo contra canónico no encontraría nada, y comparar crudo contra crudo
  // pierde los pares que difieren sólo en el relleno de ceros.
  const porGtin = gtinCanonico ? db.prepare(`SELECT id_woo,nombre,sku,gtin,stock,actualizado_en
    FROM catalogo_cache WHERE substr('00000000000000' || gtin, -14, 14)=? ORDER BY id_woo`).all(gtinCanonico) : [];
...
function activarIdentidadObservada(db, caso, pub, producto, fp, ts) {
  ...
  const actual = db.prepare(`SELECT id, activa, evidencia_fingerprint FROM identidades_canal
    WHERE canal='ml' AND external_key=? AND producto_id=? ORDER BY id DESC LIMIT 1`)
    .get(pub.clave, producto.id);
  // La identidad se refresca en cada corrida —observado_en y stock cambian—, pero el HISTORIAL
  // sólo registra cuando cambia la IDENTIDAD: es nueva, se reactivó, o cambió su huella (SKU,
  // GTIN, clave). Un cambio de stock no convierte a la publicación en otro producto.
  // Antes se anotaba una fila por clave verificada en cada auditoría: con 1130 claves y un scan
  // cada 20 minutos eran ~81.000 filas por día, y `identidad_historial` llegó a 177.286 de las
  // que 176.444 (99,5%) eran este mismo evento repetido. Eso no es historial, es telemetría —y
  // sepulta los eventos que sí importan, como una decisión o un conflicto.
  const esNovedad = !actual || Number(actual.activa) !== 1 || actual.evidencia_fingerprint !== fp;
  if (actual) {
  ...
  if (esNovedad) registrarHistorial(db, 'caso', caso.id, 'identidad_verificada', 'sistema', { ml_key: pub.clave, producto_id: producto.id }, ts);
}
...
export function auditarIdentidadProductos(db, actor = 'sistema', { lecturaConfiable = false, ahora = new Date() } = {}) {
  ...
  return db.transaction(() => {
    ...
    for (const pub of publicaciones) {
      ...
      // La cuenta de incompletas la recalcula `conciliacionIdentidad` desde la base; acá
      // sólo importa saltearla, porque sin `atributos_json` no es clasificable.
      if (pub.atributos_json === null || pub.atributos_json === undefined) continue;
      ...
      const observacionFresca = lecturaConfiable && Number.isFinite(Date.parse(pub.actualizado_en))
        && ahora.getTime() - Date.parse(pub.actualizado_en) >= 0 && ahora.getTime() - Date.parse(pub.actualizado_en) < frescuraMs(db);
    ...
    for (const pub of dormidas) {
      ...
      // en Woo, así que clasifica `sku_exacto` — y es engañoso: parece resuelta mientras hay
      // una decisión local que dice otra cosa. Se nombra el problema por lo que es.
      //
      // El caso apunta a lo que dice ML, NO a lo que dice la decisión. Una versión anterior
      // hacía lo contrario, dando por buena la decisión local, y proponía matches absurdos:
      // en `MLA1927478426|187049488547` ML dice FB-12156 (Zion Diablo Carbono 1x12, con el
      // GTIN 7798366205223 corroborándolo en tres publicaciones) y la decisión vieja decía
      // FB-22679, que es una Zion Strix 1X11v — otra bicicleta. `SELLER_SKU` es identidad;
      // una decisión histórica no la reemplaza. La contradicción se conserva y se muestra,
      // sin que el sistema elija: la resuelve una persona.
      const clasificacion = 'decision_no_aplicada';
      const skuDecidido = decisionDe.get(pub.clave)?.sku ?? null;
      const producto = match.productoWoo
        ? db.prepare('SELECT * FROM productos_fusion WHERE primary_woo_id=?').get(match.productoWoo.id_woo) : null;
      const fp = huellaIdentidad(pub, match, clasificacion);
      const caso = upsertCaso(db, pub, clasificacion, producto?.id ?? null, fp, 'urgente', ts);
      // Sin fila de evidencia la pantalla no tiene qué renderizar y muestra huecos —«Publicación: —»,
      // «SELLER_SKU: ausente»— que se leen como datos y no lo son. El pase del universo activo
      // la escribe; éste también tiene que hacerlo, y además deja asentada la decisión que
      // contradice, que es la razón de ser del caso.
      db.prepare(`INSERT OR IGNORE INTO identidad_evidencias
        (caso_id,tipo,fuente,contenido_json,fingerprint,confiable,observado_en,creado_por,creado_en)
        VALUES (?,'auditoria','sistema',?,?,?,?,?,?)`)
        .run(caso.id, json({
          ...
          woo: match.productoWoo ? { id_woo: match.productoWoo.id_woo, sku: match.productoWoo.sku, gtin: match.productoWoo.gtin } : null,
          clasificacion,
          decision_divergente: { sku: skuDecidido, origen: 'sku_matcher_decisiones' },
          observado: { stock_ml: pub.available_quantity, observado_en: pub.actualizado_en, status: pub.status },
        }), fp, lecturaConfiable ? 1 : 0, pub.actualizado_en || ts, actor, ts);
...
/**
 * Publicaciones que comparten una misma bolsa de stock de ML pero apuntan a Productos Fusion
 * distintos.
 *
 * Un `user_product` de MercadoLibre es UNA cantidad, compartida por todas las publicaciones
 * que lo referencian. Si dos de ellas están mapeadas a productos Woo distintos, cada corrida
 * del sync empuja una cantidad diferente a la MISMA bolsa: se pisan para siempre y la que
 * pierde queda exponiendo el stock de la otra.
 *
 * Verificado en producción (2026-09-05): tres pares así, y los productos eran genuinamente
 * distintos —cadena normal vs OEM, maza 28H vs 32H, horquilla Tapered vs Boost—. En el peor,
 * la bolsa tenía 4, un lado pedía 0 y el otro 4: quedaron 4 unidades a la venta con existencia
 * cero. El síntoma visible era un bucle de reconciliación cada ~3 h que logueaba `ok` sin
 * efecto; la causa no era el stock sino la identidad.
 *
 * No se auto-resuelve: cuál de los dos mapeos está mal, o si ML agrupó mal dos artículos, lo
 * decide una persona. Acá sólo se detecta y se expone.
 */
/**
 * Publicaciones que están vendiendo sin un producto Woo detrás.
 *
 * Pasa cuando una unidad Woo se da de baja, se manda a la papelera o le cambian el SKU: la
 * fila sale de `catalogo_cache` (el webhook `product.deleted` la retira) pero la publicación
 * de ML sigue activa y ofreciendo stock. Woo es la autoridad de stock; sin producto detrás no
 * hay nada que respalde lo que se está vendiendo.
 *
 * Lo que YA está cubierto y no se rehace: si entra una venta de una de estas claves, el pedido
 * **se retiene solo**. Las dos vías de cobertura de `_procesarOrden` exigen que el SKU exista
 * en el catálogo (`buscarEnCache` y `skuUnicoEnCatalogo`), así que ninguna la da por cubierta.
 *
 * Lo que faltaba, y es esto: enterarse ANTES de que llegue esa venta. Hasta acá la única señal
 * era un pedido retenido, o sea cuando el cliente ya compró.
 *
 * Detección solamente: poner el stock remoto en cero y bloquear la publicación son escrituras
 * en ML y van en una entrega aparte, con plan y canario (decisión del usuario, 2026-09-06).
 */
export function publicacionesSinRespaldoWoo(db) {
  return db.prepare(`
    SELECT p.clave, p.seller_sku, p.available_quantity, p.titulo
     ...
     ORDER BY p.available_quantity DESC, p.clave
  `).all();
}

/**
 * Códigos GTIN que más de un Producto Fusion reclama.
 *
 * La unidad de trabajo es el CÓDIGO, no la fila: en producción los 41 códigos
 * en conflicto del 2026-09-06 son 399 filas, porque a cada uno lo reclaman
 * varios productos. Resolver el código cierra todas sus filas de una.
 *
 * Se ordena por stock expuesto en ML y no por antigüedad: un código en disputa
 * con unidades a la venta puede estar descontando del producto equivocado,
 * mientras que uno sin stock es deuda de catálogo que puede esperar. Decisión
 * del usuario del 2026-09-06: se arranca por los que tienen stock.
 */
/**
 * Marca un identificador como `incorrecto`: ese código no identifica a ese
 * producto y hay que corregirlo en el canal de origen.
 *
 * Es la contraparte unitaria de `resolverConflictoIdentificador`, para el caso
 * en que el código no le corresponde a NINGÚN producto en vez de disputarse
 * entre varios. El ejemplo que la motivó: cientos de bicicletas Venzo publicadas
 * con un GTIN cuyo prefijo `455017` es de Shimano —un código de componente
 * pegado a una bicicleta—, cuando esas bicicletas no tienen código universal
 * posible.
 *
 * Sólo toca el registro local. Vaciar el atributo en ML es una escritura remota
 * por publicación, y la decisión del usuario del 2026-09-06 fue no hacerla:
 * mientras la publicación esté pausada o sin stock nadie compra contra ese
 * código, así que no justifica gastar cuota de una API que ya nos bloqueó.
 *
 * No marca un identificador `activo` que sea el único de su valor sin que se lo
 * pidan explícitamente con `permitirUnico`: dejar a un producto sin GTIN es una
 * pérdida de identidad, y tiene que ser una decisión y no un efecto colateral.
 */
export function marcarIdentificadorIncorrecto(db, productoId, valorNormalizado, actor, motivo = null, { permitirUnico = false } = {}) {
  const valor = String(valorNormalizado ?? '').trim();
  const fila = db.prepare(`SELECT * FROM identificadores_producto
    WHERE tipo='gtin' AND producto_id=? AND valor_normalizado=? AND estado IN ('activo','conflicto')`)
    .get(Number(productoId), valor);
  if (!fila) return { ok: false, code: 'NOT_FOUND', error: 'ese producto no tiene ese identificador vigente' };
  if (fila.estado === 'activo' && !permitirUnico) {
    const otros = db.prepare(`SELECT COUNT(*) n FROM identificadores_producto
      WHERE tipo='gtin' AND producto_id=? AND estado='activo' AND id<>?`).get(Number(productoId), fila.id).n;
    if (otros === 0) {
      return { ok: false, code: 'INVALID_STATE',
        error: 'es el único identificador activo del producto; hace falta confirmarlo explícitamente' };
    }
  }
  const ts = now();
  db.transaction(() => {
    db.prepare(`UPDATE identificadores_producto
      SET estado='incorrecto', resuelto_por=?, resuelto_en=?, motivo_resolucion=?, actualizado_en=? WHERE id=?`)
      .run(actor, ts, motivo, ts, fila.id);
    registrarHistorial(db, 'identificador', Number(productoId), 'gtin_marcado_incorrecto', actor, {
      gtin: valor, motivo, estado_previo: fila.estado,
    }, ts);
  })();
  return { ok: true, producto_id: Number(productoId), gtin: valor };
}

/**
 * Detalle de un código en conflicto: quiénes se lo disputan y con qué evidencia.
 *
 * La lista del resumen alcanza para priorizar pero no para decidir —da los
 * `fusion_sku` concatenados y nada más—. Para elegir un ganador hay que ver de
 * qué producto se trata y qué hay publicado con ese código.
 *
 * Se ordena por stock en ML y después por stock en Woo: el que tiene unidades
 * expuestas es el que puede estar descontando del producto equivocado. El
 * límite existe porque hay códigos comodín con más de cien productos detrás
 * (uno de fabricante llega a 117): mostrarlos todos no ayuda a decidir y
 * convierte la pantalla en un volcado.
 */
export function detalleConflictoIdentificador(db, valorNormalizado, limite = 25) {
  const valor = String(valorNormalizado ?? '').trim();
  if (!valor) return null;
  const productos = db.prepare(`
    SELECT p.id, p.fusion_sku, p.nombre_canonico, i.estado, i.fuente, i.valor_crudo, i.subtipo,
           c.stock AS stock_woo, c.sku AS sku_woo,
           COALESCE((SELECT SUM(m.available_quantity) FROM ml_publicaciones_cache m
                      WHERE m.seller_sku = c.sku AND m.status='active'), 0) AS stock_ml,
           (SELECT GROUP_CONCAT(m.clave) FROM ml_publicaciones_cache m
             WHERE m.seller_sku = c.sku
               AND substr('00000000000000' || m.gtin, -14, 14) = i.valor_normalizado) AS claves_ml
      FROM identificadores_producto i
      JOIN productos_fusion p ON p.id = i.producto_id
 LEFT JOIN catalogo_cache c ON c.id_woo = p.primary_woo_id
     WHERE i.tipo='gtin' AND i.valor_normalizado=? AND i.estado IN ('activo','conflicto')
     ORDER BY stock_ml DESC, stock_woo DESC, p.fusion_sku
     LIMIT ?
  `).all(valor, limite + 1);
  if (productos.length === 0) return null;
  const total = db.prepare(`SELECT COUNT(*) n FROM identificadores_producto
    WHERE tipo='gtin' AND valor_normalizado=? AND estado IN ('activo','conflicto')`).get(valor).n;
  return {
    valor_normalizado: valor,
    subtipo: productos[0].subtipo,
...
/**
 * Resuelve un conflicto de GTIN: un producto se queda con el código y el resto
 * queda marcado `incorrecto`.
 *
 * No se auto-resuelve por regla —lo pide el plan y lo confirman los datos: los
 * conflictos reales enfrentan productos distintos, y elegir por antigüedad o
 * por fuente elegiría mal la mitad de las veces—. La persona decide y queda
 * registrado quién y por qué.
 *
 * El descartado NO se borra ni pasa a `historico`: `incorrecto` significa que
 * ese código nunca debió estar en ese producto y hay que corregirlo en el canal
 * de origen, que es lo que dispara la tarea de catálogo. `historico` diría que
 * alguna vez fue válido, y eso sería falso.
 *
 * @returns {{ok: true, ganador: number, descartados: number}|{ok: false, code: string, error: string}}
 */
export function resolverConflictoIdentificador(db, valorNormalizado, productoGanadorId, actor, motivo = null) {
  const valor = String(valorNormalizado ?? '').trim();
  const ganadorId = Number(productoGanadorId);
  if (!valor || !Number.isInteger(ganadorId)) {
    return { ok: false, code: 'INVALID_INPUT', error: 'hace falta el identificador y el producto que se lo queda' };
  }
  const filas = db.prepare(`SELECT id, producto_id, estado FROM identificadores_producto
    WHERE tipo='gtin' AND valor_normalizado=? AND estado IN ('activo','conflicto')`).all(valor);
  if (filas.length === 0) return { ok: false, code: 'NOT_FOUND', error: 'ese identificador no está en conflicto' };
  if (filas.length === 1) {
    return { ok: false, code: 'INVALID_STATE', error: 'ese identificador ya lo reclama un solo producto' };
  }
  const ganador = filas.find((f) => f.producto_id === ganadorId);
  if (!ganador) {
    return { ok: false, code: 'INVALID_INPUT', error: 'ese producto no reclama este identificador' };
  }

  const ts = now();
  db.transaction(() => {
    // Los descartados se marcan primero: el índice único sobre `estado='activo'`
    // rechazaría al ganador mientras otra fila del mismo valor siga activa.
    for (const f of filas) {
      if (f.id === ganador.id) continue;
      db.prepare(`UPDATE identificadores_producto
        SET estado='incorrecto', resuelto_por=?, resuelto_en=?, motivo_resolucion=?, actualizado_en=?
        WHERE id=?`).run(actor, ts, motivo, ts, f.id);
    }
    db.prepare(`UPDATE identificadores_producto
      SET estado='activo', resuelto_por=?, resuelto_en=?, motivo_resolucion=?, actualizado_en=?
      WHERE id=?`).run(actor, ts, motivo, ts, ganador.id);
    registrarHistorial(db, 'identificador', ganadorId, 'conflicto_gtin_resuelto', actor, {
      gtin: valor, ganador: ganadorId,
      descartados: filas.filter((f) => f.id !== ganador.id).map((f) => f.producto_id), motivo,
    }, ts);
  })();
  return { ok: true, ganador: ganadorId, descartados: filas.length - 1 };
}

export function conflictosDeIdentificador(db) {
  return db.prepare(`
    SELECT i.valor_normalizado,
           i.subtipo,
           COUNT(DISTINCT i.producto_id) AS productos,
           GROUP_CONCAT(DISTINCT p.fusion_sku) AS fusion_skus,
           COALESCE((SELECT SUM(m.available_quantity) FROM ml_publicaciones_cache m
                      WHERE m.status='active'
                        AND substr('00000000000000' || m.gtin, -14, 14) = i.valor_normalizado), 0) AS stock_ml
      FROM identificadores_producto i
      JOIN productos_fusion p ON p.id = i.producto_id
     WHERE i.tipo='gtin' AND i.estado IN ('activo','conflicto')
     GROUP BY i.valor_normalizado
    HAVING COUNT(DISTINCT i.producto_id) > 1
     ORDER BY stock_ml DESC, i.valor_normalizado
  `).all();
}

/**
 * Claves ML que quedaron sin su producto Woo detrás, tras una baja o un cambio de SKU.
 *
 * PM-104: «una baja, papelera o alteración del SKU Woo se confirma releyendo Woo. Si persiste,
 * todas las claves ML vinculadas quedan en stock cero y bloqueadas y se retienen los pedidos
 * afectados». Esta es la mitad que NO escribe en ML: confirma, marca y retiene. Poner stock
 * cero y bloquear es una escritura remota y va en una entrega aparte, con canario.
 *
 * Se buscan por dos vías porque cubren huecos distintas: la identidad activa es la vinculación
 * que Fusion verificó, pero una publicación puede llevar el SKU sin haber llegado nunca a
 * verificarse. Dejarla afuera sería no proteger justo la que nadie miró.
 */
export function clavesAfectadasPorBajaWoo(db, { idWoo = null, sku = null } = {}) {
  const claves = new Map();
  if (idWoo != null) {
    for (const r of db.prepare(`SELECT ic.external_key AS clave, p.id AS producto_id, p.fusion_sku
        FROM identidades_canal ic
        JOIN productos_fusion p ON p.id = ic.producto_id
       WHERE ic.canal='ml' AND ic.activa=1 AND p.primary_woo_id=?`).all(Number(idWoo))) {
      claves.set(r.clave, r);
    }
  }
  const skuLimpio = String(sku ?? '').trim();
  if (skuLimpio) {
    for (const r of db.prepare(`SELECT m.clave, NULL AS producto_id, NULL AS fusion_sku
        FROM ml_publicaciones_cache m
       WHERE TRIM(COALESCE(m.seller_sku,''))=? AND m.status='active'
         AND COALESCE(m.available_quantity,0)>0 AND ${FILTRO_MARKETPLACE}`).all(skuLimpio)) {
      if (!claves.has(r.clave)) claves.set(r.clave, r);
    }
  }
...
/**
 * Abre la protección tras confirmar que el producto Woo ya no está.
 *
 * `confirmado` es obligatorio y lo decide el llamador: un `product.deleted` firmado por Woo ya
 * es confirmación, pero un cambio de SKU exige releer. Sin esa distinción, un webhook espurio
 * o una lectura a medias retendría pedidos de productos que están perfectamente vivos.
 *
 * Idempotente: repetir el mismo evento actualiza la detección y no duplica casos ni
 * retenciones. Los webhooks de Woo llegan repetidos con normalidad.
 */
export function protegerPorBajaWoo(db, { idWoo = null, sku = null, motivo = 'baja_woo', confirmado = false }, actor = 'sistema') {
  if (!confirmado) {
    return { ok: false, code: 'INVALID_STATE', error: 'la baja de Woo tiene que estar confirmada antes de proteger' };
  }
  const afectadas = clavesAfectadasPorBajaWoo(db, { idWoo, sku });
  if (afectadas.length === 0) return { ok: true, claves: 0, casos: 0, operaciones: 0, pedidos_retenidos: 0 };

  const ts = now();
  const cfg = db.prepare('SELECT modo, escrituras_remotas_habilitadas FROM identidad_config WHERE id=1').get();
  let casos = 0;
  let operaciones = 0;
  let pedidos = 0;
  db.transaction(() => {
    for (const a of afectadas) {
      const fp = fingerprintEvidencia({ v: 1, motivo, id_woo: idWoo, sku, clave: a.clave });
      const previo = db.prepare("SELECT id FROM identidad_casos WHERE direccion='woo_ml' AND ml_key=? AND resuelto_en IS NULL").get(a.clave);
      if (previo) {
        db.prepare('UPDATE identidad_casos SET ultima_deteccion_en=?, evidencia_fingerprint=? WHERE id=?')
          .run(ts, fp, previo.id);
        continue;
      }
      db.prepare(`INSERT INTO identidad_casos
        (direccion,ml_key,producto_id,clasificacion,estado,severidad,evidencia_fingerprint,expected_version,primera_deteccion_en,ultima_deteccion_en)
        VALUES ('woo_ml',?,?,?,'urgente','critica',?,1,?,?)`)
        .run(a.clave, a.producto_id, motivo, fp, ts, ts);
      casos += 1;
      const casoId = db.prepare("SELECT id FROM identidad_casos WHERE direccion='woo_ml' AND ml_key=? AND resuelto_en IS NULL").get(a.clave)?.id;
      registrarHistorial(db, 'caso', casoId ?? null, 'woo_baja_protegida', actor, {
        clave: a.clave, id_woo: idWoo, sku, motivo, fusion_sku: a.fusion_sku,
      }, ts);
      // La operación nace en `shadow` salvo que las escrituras remotas estén habilitadas, igual
      // que la corrección de SKU: desplegar esto no enciende nada por sí solo. El worker aplica
      // después sus propios frenos —canario y lote— antes de tocar ML.
      const estadoOp = cfg?.modo === 'enforced' && Number(cfg?.escrituras_remotas_habilitadas) === 1 ? 'pendiente' : 'shadow';
      db.prepare(`INSERT INTO identidad_operaciones
        (operation_id,tipo,caso_id,decision_id,producto_id,ml_key,sku_anterior,sku_objetivo,stock_objetivo,
         estado,paso_actual,iniciada_en,proximo_intento_en,actualizada_en)
        VALUES (?,'proteccion_woo',?,NULL,?,?,?,NULL,0,?, 'zero',?,?,?)`)
        .run(`prot-${a.clave}-${ts}`, casoId, a.producto_id, a.clave, sku || null, estadoOp, ts, ts, ts);
      operaciones += 1;
    }
    // El vínculo pedido→producto es por SKU: `pedidos_cache.items_json` guarda el `sku` de cada
    // línea y no el `item_id` de ML, así que se busca por ahí. Sin SKU no hay nada que retener.
    // Los pedidos ya sincronizados a Woo (`wc_order_id<>0`) no se tocan: el cliente compró y la
    // unidad salió; retenerlos acá no devuelve nada y sí frena una preparación en curso.
    const skuAfectado = String(sku ?? '').trim();
    // `pedidos_cache` la crea `routes/preparacion.js` al montarse, no `db/schema.sql`. Si no
    // está, no es que falte información: es que todavía no hay ningún pedido registrado, así
    // que no hay nada que retener. Preguntar es más barato que acoplarse al orden de arranque.
    const hayPedidos = !!db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='pedidos_cache'").get();
    if (skuAfectado && hayPedidos) {
      for (const o of db.prepare(`SELECT DISTINCT p.ml_order_id
          FROM pedidos_cache p, json_each(p.items_json) j
          JOIN ordenes_ml_wc_pedidos o ON o.ml_order_id = p.ml_order_id
         WHERE p.canal='ml' AND json_extract(j.value,'$.sku')=?
           AND o.wc_order_id=0 AND o.retenido_en IS NULL AND o.cancelado_en IS NULL`).all(skuAfectado)) {
        const existe = db.prepare("SELECT 1 FROM guardia_ml_pedidos_retenidos WHERE ml_order_id=? AND estado='retenido'").get(String(o.ml_order_id));
        if (existe) continue;
        db.prepare(`INSERT INTO guardia_ml_pedidos_retenidos (ml_order_id,motivo,items_json,estado,creado_en,actualizado_en)
          VALUES (?,?,?,'retenido',?,?)`)
          .run(String(o.ml_order_id), motivo, JSON.stringify([{ sku: skuAfectado, id_woo: idWoo }]), ts, ts);
        pedidos += 1;
      }
    }
  })();
  return { ok: true, claves: afectadas.length, casos, operaciones, pedidos_retenidos: pedidos };
}

/**
 * Archiva las identidades ML cuya publicación ya no existe en MercadoLibre.
 *
 * Una publicación dada de baja deja su identidad activa apuntando a nada: cuenta como cobertura,
 * ocupa el tablero y nunca se puede verificar, porque no hay qué leer.
 *
 * Sólo corre después de un scan completo y confiable, y ésa es toda la seguridad del asunto: el
 * criterio es «no está en el cache», así que con un refresco acotado o interrumpido archivaría
 * identidades perfectamente vivas que simplemente no se leyeron esta vez. `lecturaConfiable` no
 * tiene default a propósito —quien llame tiene que afirmarlo—.
 *
 * No borra: `activa=0` y `archivado_en`. Si la publicación reaparece, la reactivación existente
 * la vuelve a poner en pie con su evidencia.
 */
export function archivarIdentidadesMlHuerfanas(db, { lecturaConfiable = false, actor = 'sistema' } = {}) {
  if (!lecturaConfiable) return { ok: false, code: 'LECTURA_NO_CONFIABLE', archivadas: 0 };
  const huerfanas = db.prepare(`SELECT i.id, i.external_key, i.producto_id FROM identidades_canal i
     WHERE i.canal='ml' AND i.activa=1
       AND NOT EXISTS (SELECT 1 FROM ml_publicaciones_cache m WHERE m.clave = i.external_key)`).all();
  if (huerfanas.length === 0) return { ok: true, archivadas: 0 };
  const ts = now();
  db.transaction(() => {
    for (const h of huerfanas) {
      db.prepare(`UPDATE identidades_canal SET activa=0, archivado_en=?, actualizado_en=?,
        expected_version=expected_version+1 WHERE id=?`).run(ts, ts, h.id);
      registrarHistorial(db, 'identidad', h.id, 'identidad_ml_archivada', actor, {
        clave: h.external_key, producto_id: h.producto_id, motivo: 'publicación ausente del cache tras scan confiable',
      }, ts);
    }
  })();
  return { ok: true, archivadas: huerfanas.length };
}

/**
 * Purga las repeticiones de `identidad_verificada`, conservando la primera y la última de cada
 * caso.
 *
 * El historial es append-only por diseño, y ese invariante existe para proteger las decisiones:
 * una corrección agrega eventos y nunca reescribe los anteriores. Lo que se borra acá no son
 * decisiones sino telemetría automática —hasta el 2026-09-06 se anotaba una fila por clave
 * verificada en CADA auditoría, ~81.000 por día, y 176.444 de las 177.286 filas de la tabla eran
 * ese mismo evento—. Decisión del usuario del 2026-09-06.
 *
 * Se conservan los dos extremos de cada caso a propósito: la primera dice desde cuándo esa
 * identidad está verificada, la última hasta cuándo se la vio bien. Con esos dos puntos el
 * historial sigue contestando lo que se le pregunta; las 175.216 repeticiones del medio no
 * agregaban nada.
 *
 * No toca ningún otro evento: una decisión, un conflicto o un archivado quedan intactos.
 */
export function purgarHistorialVerificaciones(db, { simular = true } = {}) {
  const total = db.prepare("SELECT COUNT(*) n FROM identidad_historial WHERE evento='identidad_verificada'").get().n;
  const extremos = db.prepare(`SELECT MIN(id) a, MAX(id) b FROM identidad_historial
    WHERE evento='identidad_verificada' GROUP BY entidad_id`).all();
  const conservar = new Set();
  for (const e of extremos) { conservar.add(e.a); conservar.add(e.b); }
  const aBorrar = total - conservar.size;
  if (simular || aBorrar <= 0) return { simulado: simular, total, conservados: conservar.size, borrados: 0, a_borrar: Math.max(0, aBorrar) };
  const borradas = db.transaction(() => db.prepare(`DELETE FROM identidad_historial
    WHERE evento='identidad_verificada'
      AND id NOT IN (SELECT MIN(id) FROM identidad_historial WHERE evento='identidad_verificada' GROUP BY entidad_id)
      AND id NOT IN (SELECT MAX(id) FROM identidad_historial WHERE evento='identidad_verificada' GROUP BY entidad_id)`).run().changes)();
  return { simulado: false, total, conservados: conservar.size, borrados: borradas };
}

/** Claves esperando la protección remota, para el tablero de salud. */
export function clavesEsperandoProteccion(db) {
  return db.prepare(`SELECT c.ml_key, c.clasificacion, c.primera_deteccion_en,
      m.titulo, m.seller_sku, m.available_quantity, m.status
    FROM identidad_casos c
    LEFT JOIN ml_publicaciones_cache m ON m.clave = c.ml_key
   WHERE c.direccion='woo_ml' AND c.resuelto_en IS NULL
   ORDER BY COALESCE(m.available_quantity,0) DESC, c.primera_deteccion_en`).all();
}

export function conflictosDeBolsaCompartida(db) {
  // Se agrupa por el producto del CASO, no por la identidad activa en `identidades_canal`:
  // una identidad sólo se activa cuando el caso queda verificado, y estos conflictos impiden
  // justamente que se verifique —el stock nunca coincide porque las dos publicaciones se lo
  // pisan—. Apoyarse en la identidad activa haría que el detector no viera ni uno solo de los
  // casos reales de producción.
  return db.prepare(`
    SELECT p.user_product_id,
           COUNT(DISTINCT c.producto_id) AS productos,
           GROUP_CONCAT(DISTINCT p.clave)      AS claves,
           GROUP_CONCAT(DISTINCT p.seller_sku) AS skus
      FROM ml_publicaciones_cache p
      JOIN identidad_casos c ON c.direccion='ml_fusion' AND c.ml_key=p.clave AND c.producto_id IS NOT NULL
     WHERE p.user_product_id IS NOT NULL
       AND ${FILTRO_MARKETPLACE}
       -- El caso conserva su producto_id aunque la publicacion pierda el SELLER_SKU en ML.
       -- Sin esta condición se reporta como conflicto un par que ya no reclama nada: pasó el
       -- 2026-09-06 con el par de horquillas, cuyas dos publicaciones quedaron sin SKU en ML
       -- (verificado contra la API: ML tampoco lo tiene) y seguían saliendo por el caso viejo.
       AND TRIM(COALESCE(p.seller_sku,'')) <> ''
     GROUP BY p.user_product_id
    HAVING COUNT(DISTINCT c.producto_id) > 1
     ORDER BY productos DESC, p.user_product_id
  `).all();
}
...
export function conciliacionIdentidad(db) {
  ...
  // Los mismos tres estados que lista la cola de trabajo (`listarCasosIdentidad` con
  // `pendientes`). Antes esto incluía además `pendiente`, y entonces el contador decía que
  // había más trabajo humano del que la cola mostraba: un caso ya decidido, esperando al
  // worker, seguía contando como si alguien tuviera que mirarlo.
  const urgentes = db.prepare(`SELECT COUNT(*) n FROM identidad_casos c WHERE ${enUniverso}
    AND c.estado IN ('urgente','tomado','intervencion')`).get().n;
  ...
  // `esperando_operacion` entra en la suma aunque no sea trabajo humano: el gate comprueba que
  // toda clave auditada esté en ALGUNA categoría. Sacarlo de `urgentes` sin sumarlo acá dejaría
  // un agujero por el que el tablero anunciaría "conciliado" con casos sin resolver.
  const conciliado = total > 0 && incompletas === 0
    && auditadas === verificadas + excepciones + urgentes + esperandoOperacion;
...
export function estadoIdentidadProductos(db, ahora = new Date()) {
  ...
  const degradado = !Number.isFinite(ultimo) || ahora.getTime() - ultimo >= frescuraMs(db) || !!config?.ultimo_scan_error;
...
export function obtenerCasoIdentidad(db, id) {
  ...
  const hermanasSku = skuPub
    ...
    : [];
  const gtinProd = producto ? claveGtin(producto.gtin_woo) : null;
...
export function buscarProductosFusion(db, { q = '', limite = 20, gtin_ml: gtinMl, sku_ml: skuMl } = {}) {
  ...
  const gtinRef = claveGtin(gtinMl);
  ...
  for (const f of filas) {
    ...
    const coincideGtin = !!gtinRef && claveGtin(f.gtin) === gtinRef;
...
export async function procesarPasoOperacionIdentidad(db, operacionId, adapter, { allowRemoteWrites = false } = {}) {
  ...
  try {
    ...
    if (paso === 'zero' && op.tipo === 'proteccion_woo') {
      // Sin atajos por SKU: acá no se persigue una identidad sino apagar la exposición. Si ML
      // ya está en cero, se confirma igual en `verify_zero` y la operación termina sin escribir.
    ...
    const secuencia = pasosDe(op);
    const siguiente = secuencia[secuencia.indexOf(paso) + 1];
