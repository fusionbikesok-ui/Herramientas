...
export function conciliacionIdentidad(db) {
  ...
  return { total, auditadas, observacion_incompleta: incompletas, verificadas, excepciones, urgentes, conciliado };
}
...
export function buscarProductosFusion(db, { q = '', limite = 20 } = {}) {
  ...
  return db.prepare(`SELECT p.id,p.nombre_canonico,p.fusion_sku,p.estado,p.primary_woo_id,
    ...
    ORDER BY p.nombre_canonico COLLATE NOCASE,p.id LIMIT ?`)
    .all(...params).map((o) => ({ ...o, img: imagenSegura(o.img) }));
}
