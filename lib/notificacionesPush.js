...
export function validarConfiguracionPush(env = process.env) {
  ...
  if (provider && !['mock', 'fcm'].includes(provider)) {
    throw new Error('PUSH_PROVIDER debe ser mock o fcm');
  }
  if (env.NODE_ENV === 'production' && provider !== 'fcm') {
    throw new Error('En producción PUSH_PROVIDER=fcm es obligatorio; mock no está permitido');
  }
