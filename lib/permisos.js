...
export const HERRAMIENTAS = [
  ...
  { id: 'stock-exceptions', label: 'Excepciones de stock', niveles: true },
  ...
  // (WC→ML) era un permiso aparte, niveles:false — se absorbió acá. La migración
  ...
  { id: 'notificaciones-ml', label: 'Notificaciones ML', niveles: false },
];
...
const REGLAS = [
  ...
  { re: /^\/nuevos-productos\/operaciones\/[^/]+\/conciliar$/, resolve: () => ({ anyOf: ['stock', 'recepcion'], nivel: 'write' }) },
  { re: /^\/nuevos-productos\/(categorias-woo|operaciones\/[^/]+)$/, resolve: () => ({ anyOf: ['stock', 'recepcion'], nivel: 'read' }) },
  { re: /^\/nuevos-productos\/crear-borrador$/, resolve: () => ({ anyOf: ['stock', 'recepcion'], nivel: 'write' }) },
  ...
  // Guardia ML comparte la autorización de Matcher, pero sus acciones además
  // restringen el rol operativo en el router específico.
  // Bandeja de identidad (E3): decidir un caso es escribir sobre el matcher, así que POST pide matcher:write.
  { re: /^\/bandeja-identidad(\/|$)/, resolve: (m) => ({ anyOf: ['matcher'], nivel: nivelDe(m) }) },
  { re: /^\/guardia-ml(\/|$)/,  resolve: (m) => ({ anyOf: ['matcher'], nivel: nivelDe(m) }) },
  // Identidad de productos comparte Matcher. Notas/evidencia narrativa son colaboración
  // de lectura; decisiones, claims y demás mutaciones requieren matcher:write.
  { re: /^\/identidad-productos\/casos\/\d+\/notas$/, methods: ['POST'], resolve: () => ({ anyOf: ['matcher'], nivel: 'read' }) },
  { re: /^\/identidad-productos(\/|$)/, resolve: (m) => ({ anyOf: ['matcher'], nivel: nivelDe(m) }) },
  ...
  // Gestión relacional de pedidos: lectura para consulta/recuperación, escritura para
  // cambios de pedido, contactos y derivaciones operativas.
  { re: /^\/gestion-pedidos(\/|$)/, resolve: (m) => ({ anyOf: ['pedidos'], nivel: nivelDe(m) }) },
  ...
  { re: /^\/stock-exceptions(\/|$)/, resolve: (m) => ({ anyOf: ['stock-exceptions', 'stock'], nivel: nivelDe(m) }) },
  { re: /^\/warranties(\/|$)/, resolve: (m) => ({ anyOf: ['pedidos', 'stock-exceptions'], nivel: nivelDe(m) }) },
  { re: /^\/workshop(\/|$)/, resolve: (m) => ({ anyOf: ['pedidos', 'stock-exceptions'], nivel: nivelDe(m) }) },
  ...
  // ── Notificaciones ML: preguntas y mensajes sin responder (permiso binario, como etiquetas) ──
  { re: /^\/notificaciones-ml(\/|$)/, resolve: () => ({ anyOf: ['notificaciones-ml'] }) },
];
