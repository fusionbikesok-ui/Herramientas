...
const REGLAS = [
  ...
  // ── Cobertura ──
  // `cobertura` es niveles:false → la UI de Usuarios solo otorga un checkbox de "acceso"
  // (guarda nivel:'read' siempre, no hay selector read/write — ver el comentario de
  // `/inventario` más abajo, mismo criterio EXACTO). Antes de la reescritura a herramienta
  // accionable esto casi no importaba (la pantalla era un informe con un único POST de
  // exclusiones); ahora confirmar/descartar/saltear/publicar/pausar/desvincular/deshacer SON
  // la herramienta entera, y son todos método POST/PATCH/DELETE → `nivelDe(m)` los resolvía
  // como 'write', que ningún operario no-admin puede tener con niveles:false. Resultado real
  // (hallazgo del `revisor`): un operario con el permiso de Cobertura otorgado no podía usar
  // ni un solo botón — 403 en todo menos las lecturas. Mismo bug que ya costó el Contador de
  // Inventario v2. Fix: nivel:'read' fijo, igual que /inventario.
  { re: /^\/cobertura(\/|$)/,   resolve: () => ({ anyOf: ['cobertura'], nivel: 'read' }) },
