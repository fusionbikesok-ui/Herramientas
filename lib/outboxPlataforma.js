...
export function traducirEvento({ evento_id, tipo, payload, creado_en }) {
  if (tipo === 'matcher.decision') {
    ...
    const sistema = payload.confirmado_por === 'sistema' || Object.hasOwn(ORIGENES_SISTEMA, payload.origen ?? '');
    return { ruta: RUTA_EVENTOS_CATALOGO, cuerpo: {
      ...
      motivo: payload.origen ? (ORIGENES_SISTEMA[payload.origen] ?? payload.origen) : null,
      confirmado_por: sistema ? null : (payload.confirmado_por || null),
