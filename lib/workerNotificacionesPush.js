  ...
  try {
    ...
    for (const dev of dispositivos) {
      try {
        ...
        if (estadoPrevio) {
          // ALTO 1 (5ª): Si es 'agotado' Y tipo='reaviso' para incidente activo, resetear
          if (estadoPrevio.estado === 'agotado' && tipo === 'reaviso' && inc.estado === 'activo') {
            // Resetear la fila para permitir un nuevo ciclo de reaviso
            db.prepare(`
              UPDATE notificaciones_enviadas
            ...
            console.log(
              `[push-worker] reaviso agotado reseteado para dispositivo ${dev.id}/${inc.id} (nuevo ciclo de reaviso)`
            );
            // Continuar a intentar este dispositivo (no usar continue)
          } else if (estadoPrevio.estado === 'agotado') {
            // ALTO 1 FIX: Si es 'agotado' pero NO es 'reaviso' o incidente no activo, saltar
            console.warn(
              `[push-worker] dispositivo ${dev.id} ya está agotado para ${tipo}/${inc.id} — no reintentar`
            );
            continue;
          } else {
            // Estado es 'fallido' — evaluar backoff y reintentos
