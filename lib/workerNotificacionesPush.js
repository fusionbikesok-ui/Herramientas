...
import { revocarPorRespuesta } from './pushTokens.js';
  ...
  try {
    ...
    for (const dev of obtenerDispositivosDelUsuario(db, userId)) {
      try {
        ...
        const resultado = await enviarNotificacion(dev.token, { ...payload, idempotencia_key: idempotencia }, { entorno: dev.entorno });
        registrarResultado(db, reserva.fila, resultado);
        revocarPorRespuesta(db, dev.token, resultado);
      } catch (err) {
  ...
  try {
    ...
    for (const fila of fallidas) {
      try {
        ...
        const resultado = await enviarNotificacion(device.token, { ...payload, idempotencia_key: fila.idempotencia || String(fila.id) }, { entorno: device.entorno });
        registrarResultado(db, { ...fila, estado: 'pendiente' }, resultado);
        revocarPorRespuesta(db, device.token, resultado);
