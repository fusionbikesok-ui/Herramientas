 ...
 * false si chocó con UNIQUE constraint o fue actualizada.
 */
 ...
 * Opciones elegida: Opción A del revisor — UPDATE en vez de nueva fila.
 */
function crearNotificacionUsuario(db, userId, tipo, payload, incidenteId) {
  ...
  try {
    ...
    if (err.message.includes('UNIQUE')) {
      ...
      if (tipo === 'reaviso') {
        // ALTO 4: 'reaviso' se permite repetir — actualizar la fila existente
        // con el estado más reciente
        db.prepare(`
          UPDATE notificaciones_usuario
          SET titulo = ?, cuerpo = ?, creado_en = ?
