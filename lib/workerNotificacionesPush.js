 ...
 *
 * ALTO 4 fix: Incrementa contador de intentos en reintentos fallidos.
 *
 * Retorna: { insertado: boolean } — true si la fila fue insertada realmente,
 * false si chocó con UNIQUE constraint.
 */
function registrarIntentoDeSend(db, deviceTokenId, tipo, incidenteId, resultado) {
  const ts = now();

  // Para reintentos fallidos, leer intentos de la fila anterior (ya fue deleteda arriba si procede)
  const ultimoIntento = db
    .prepare(`
      SELECT intentos FROM notificaciones_enviadas
      WHERE device_token_id = ? AND tipo = ? AND incidente_id = ?
      ORDER BY creado_en DESC LIMIT 1
    `)
    .get(deviceTokenId, tipo, incidenteId);

  const proximoNumeroIntento = (ultimoIntento?.intentos || 0) + 1;
 ...
 *
 * BLOQUEANTE 2: usa INSERT OR IGNORE porque el índice único (user_id, tipo, incidente_id)
 * puede chocar si el usuario ya vio este tipo de notificación para este incidente
 * (en un reintento o second-pass del worker).
 */
function crearNotificacionUsuario(db, userId, tipo, payload, incidenteId) {
  ...
  try {
    ...
    if (err.message.includes('UNIQUE')) {
      // Ya existe notificación para este usuario/tipo/incidente — omitir (ya tiene la notificación)
    } else {
...
async function procesarIncidente(db, inc) {
  try {
    ...
    if (inc.estado === 'activo') {
      // Buscar si hay ALGUNA notificación previa (de cualquier tipo, enviada)
      const hayNotificacionPrevia = db
        .prepare(`
      ...
      if (!hayNotificacionPrevia) {
        // Sin notificación previa → enviar 'nuevo' (y SOLO 'nuevo', no 'reaviso')
        tiposAEnviar.push('nuevo');
      } else {
        // Hay notificación previa → evaluar 'reaviso'
 ...
 *
 * ALTO 4 fix: evaluar backoff antes de reintentar dispositivos con fallos previos.
 */
async function enviarADispositivosDelUsuario(db, userId, inc, tipo) {
  try {
    ...
    for (const dev of dispositivos) {
      try {
        // ALTO 4: si hay fallo previo, verificar backoff antes de reintentar
        ...
        if (falloAnterior) {
          // Hay fallo previo — evaluar backoff
          ...
          // Backoff cumplido — proceder a reintentar (deletear fila vieja e insertar nueva)
          db.prepare(`
