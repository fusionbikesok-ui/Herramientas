...
import { CATEGORIA_SISTEMA } from './pushCasoPayload.js';
import { esAvisoVentaRetenida, RECORDATORIO_VENTA_RETENIDA_MIN } from './guardiaAvisos.js';
 ...
 */
/**
 * Los incidentes de integración son avisos de SISTEMA: informan, no piden acción. Declarar la
 * categoría importa porque iOS resuelve los botones por identificador, y sin ella un aviso de
 * "la conexión se degradó" podría heredar los botones de un caso de cliente.
 */
function payloadDelIncidente(inc, tipo) {
  // Venta retenida por Guardia ML: pide acción (liberar o vincular), no informa una degradación.
  if (esAvisoVentaRetenida(inc)) {
    const titulos = { nuevo: '🛑 Venta retenida en Guardia ML', reaviso: '🔔 Sigue retenida: venta en Guardia ML', resuelto: '✅ Venta liberada' };
    return {
      titulo: titulos[tipo] || 'Guardia ML',
      cuerpo: tipo === 'resuelto' ? 'La venta ya no está retenida.' : (inc.mensaje_humano || 'Una venta pagada no llega a Woo hasta resolverla en Guardia ML.'),
...
function obtenerUsuariosParaNotificar(db, inc) {
  // Venta retenida: la reciben quienes pueden liberarla (misma regla que `puedeResolver` en
  // routes/guardiaMl.js: admin o Matcher con escritura), sin depender de la severidad.
  if (esAvisoVentaRetenida(inc)) {
    return db
      .prepare(`
        ...
        WHERE u.activo = 1
          AND (u.is_admin = 1 OR EXISTS (
            ...
            WHERE up.user_id = u.id AND up.herramienta = 'matcher' AND up.nivel = 'write'
          ))
      `)
      .all();
  }
  ...
  try {
    ...
    if (inc.estado === 'activo') {
      ...
      if (!hayNotificacionEnFeed) {
        ...
        if (esAvisoVentaRetenida(inc)) {
          // Un único recordatorio, a RECORDATORIO_VENTA_RETENIDA_MIN del aviso inicial.
          const yaRecordado = db
            .prepare("SELECT 1 FROM notificaciones_usuario WHERE user_id = ? AND incidente_id = ? AND tipo = 'reaviso' LIMIT 1")
            .get(userId, inc.id);
          const aviso = db
            .prepare("SELECT creado_en FROM notificaciones_usuario WHERE user_id = ? AND incidente_id = ? AND tipo = 'nuevo' LIMIT 1")
            .get(userId, inc.id);
          const vence = new Date(Date.now() - RECORDATORIO_VENTA_RETENIDA_MIN * 60 * 1000).toISOString();
          if (!yaRecordado && aviso && aviso.creado_en <= vence) tiposAEnviar.push('reaviso');
        } else {
...
async function enviarADispositivosDelUsuarioSeguro(db, userId, inc, tipo) {
  try {
    const payload = { ...payloadDelIncidente(inc, tipo), categoryId: CATEGORIA_SISTEMA };
  ...
  try {
    ...
    for (const fila of fallidas) {
      try {
        ...
        const payload = { ...payloadDelIncidente(inc, fila.tipo), categoryId: CATEGORIA_SISTEMA };
