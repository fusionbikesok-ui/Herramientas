...
/** Colapsa listas largas de ids (multiget) y trunca, para que el log sea legible. */
function _normalizarPath(path) {
  let p = String(path || '');
  p = p.replace(/([?&]ids=)([^&]*)/i, (_, pre, val) => {
    const n = val ? val.split(',').length : 0;
    return `${pre}<${n} ids>`;
  });
  const MAX_LEN = 150;
  if (p.length > MAX_LEN) p = `${p.slice(0, MAX_LEN)}…(truncado)`;
  return p;
}

// Deduplicación por ventana: la primera aparición de (método, path normalizado,
...
const _dedupErrores = new Map(); // clave -> { repeticiones, timer }

function _flushDedup(clave) {
  const entrada = _dedupErrores.get(clave);
  if (!entrada) return;
  if (entrada.repeticiones > 0) {
    console.error(`[ML][error] ${clave} — ${entrada.repeticiones} veces más en los últimos 10 min`);
  }
...
export function estadoErroresMl() {
  return {
    ...
    porRecurso: Object.fromEntries(_contadoresSinteticosPorRecurso),
  };
}
...
function _activarCooldown(retryAfterHeader, motivo = '429', contexto = null) {
  ...
  const ctxTxt = contexto ? ` (${String(contexto.method).toUpperCase()} ${_normalizarPath(contexto.path)})` : '';
...
export async function mlFetch(db, cfg, method, path, body = null, opts = {}) {
  ...
  if (resp.status === 429) {
    ...
    _activarCooldown(resp.headers?.['retry-after'], '429 real de ML', { method, path });
