...
import { medirLlamadaMl } from './medicionMl.js';
...
export async function mlFetch(db, cfg, method, path, body = null, opts = {}) {
  ...
  if (!opts.manual && _cooldownActivo()) {
    ...
    medirLlamadaMl(db, recurso, { sintetica: true });
  ...
  let resp;
  try {
    resp = await _request(token, method, path, body, opts.headers);
    medirLlamadaMl(db, recurso, { status: resp.status });
  } catch (e) {
    medirLlamadaMl(db, recurso, { status: 0 });
...
async function _request(token, method, path, body, extra = null) {
  return axios.request({
    url: ML_API + path,
    method,
    // `extra` (p. ej. x-format-new del gateway E1) va primero: nunca puede pisar Authorization.
