...
/**
 * Activa o escala el cooldown global tras un 429 real de ML.
 * Si ML manda Retry-After y es mayor al escalón calculado, se respeta ese valor.
 */
function _activarCooldown(retryAfterHeader) {
  ...
  console.error(`[ML] 429 recibido — cooldown activado por ${Math.round(esperaMs / 1000)}s (nivel ${_backoffNivel})`);
}
 ...
 * No conectado a ninguna ruta/frontend todavía — solo disponible para uso futuro.
 */
...
async function _doRefresh(db, cfg, refreshToken) {
  ...
  try {
    ...
    _activarCooldown();
    throw new Error('Error de conectividad con MercadoLibre al refrescar el token — cooldown activado, reintentará solo');
  }
  ...
  if (resp.status !== 200) {
    if (resp.status === 429) {
      ...
      throw new Error('Error transitorio de rate limit ML (429) al refrescar el token — reintentará solo tras el cooldown');
    }
    if (resp.status === 400 || resp.status === 401) {
      ...
      throw new Error(`Autenticación ML rechazada (${resp.status}) — revisar client_secret o rehacer la autorización OAuth`);
    }
    if (resp.status >= 500) {
      // 5xx sostenido de ML: mismo tratamiento que el 429, para no martillar un
      // servicio caído ciclo tras ciclo con los 9 crons.
      _activarCooldown();
      throw new Error(`Error de servidor ML (${resp.status}) al refrescar el token — cooldown activado, reintentará solo`);
    }
    throw new Error(`Autenticación ML rechazada (${resp.status}) — revisar client_secret o rehacer la autorización OAuth`);
  }
