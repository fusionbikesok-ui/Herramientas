...
// las llamadas de los crons hasta que expire. Las llamadas manuales (opts.manual)
// nunca lo respetan.
...
// 60s de margen, pero en la práctica un cooldown largo puede solapar reintentos
// de refresh también afectados por el 429 — ver rama worktree-fix-token-ml-429).
...
function _activarCooldown(retryAfterHeader) {
  const yaHabiaCooldown = _cooldownActivo();
  ...
  if (Number.isFinite(retryAfter) && retryAfter > 0 && retryAfter * 1000 > esperaMs) {
...
/** Resetea el backoff tras una respuesta exitosa. */
