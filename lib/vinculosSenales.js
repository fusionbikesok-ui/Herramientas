 ...
 *
 * El prefijo solo cuenta si lo que sigue es un separador (espacio, paréntesis, etc.) o el fin
 * de la cadena — nunca otro caracter alfanumérico. Sin esta condición "S" sería prefijo de
 * "SM" y dos talles distintos se tomarían como coincidentes, justo el error que esta señal
...
function atributoCoincide(ml, wc) {
  ...
  const resto = largo.slice(corto.length);
  return resto === '' || /^[^a-z0-9]/.test(resto);
}
...
export function senalesDeVinculo(fila) {
  ...
  if (!colorOk || !talleOk) {
    ...
    senales.push({
      ...
      valor: `${normalizarAtributo(fila.color)}|${normalizarAtributo(fila.talle)}|${normalizarAtributo(wc.color)}|${normalizarAtributo(wc.talle)}`,
    });
  }
  ...
  if (precioMl > 0 && precioWc > 0) {
    ...
    if (desvio > UMBRAL_DESVIO_PRECIO) {
      senales.push({
        ...
        valor: `${Math.round(precioMl)}|${Math.round(precioWc)}`,
      });
    }
  }
