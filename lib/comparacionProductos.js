...
const TTL_BORRADO_MS = 3600e3;

async function leerProducto(db, mlCfg, id) {
  const hit = db.prepare('SELECT * FROM ml_productos_cache WHERE product_id=?').get(id);
  if (hit && Date.now() - Date.parse(hit.leido_en) < (hit.http_status === 404 ? TTL_BORRADO_MS : TTL_MS)) return { ...hit, cache: true };
  let r;
...
export async function compararProductos(db, mlCfg, cambio, publicacion) {
  ...
  return { ok: true, viejo: vista(cambio.valor_anterior, viejo || {}), nuevo: vista(cambio.valor_nuevo, nuevo || {}), diferencias, veredicto, comparados: comparables };
}
