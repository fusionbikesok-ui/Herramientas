...
export function validarAccionesPermisos(acciones = []) {
  const invalidas = acciones.filter(accion => !accion?.usuario || accion.autorizado !== true).map(accion => ({
    accion: accion?.accion || 'desconocida', usuario: accion?.usuario || null,
  }));
  return { ok: invalidas.length === 0, total: acciones.length, invalidas };
}
