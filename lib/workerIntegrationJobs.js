...
import { ingerirPregunta, ingerirReclamo, reconciliarMensajesMl } from '../routes/notificacionesMl.js';
...
/**
 * `sinJob` persiste el recibo SIN encolar trabajo. Lo usa la rama de cuenta ajena (server.js): de esa
 * notificación queremos la traza, no procesarla — encolar un job sobre el recurso de otra cuenta sería
 * trabajar para un tercero que puede elegir el user_id. Sin job, el recibo nace `completed`.
 */
export function registrarWebhookMl(db, envelope, { sinJob = false, sombra = false } = {}) {
  ...
  // `messages` queda AUDITADO, sin proyección (§15 del plan pedía decidir por topic; se decide
  // acá). No es una preferencia: no se puede proyectar. El webhook manda el `resource` como id
  // pelado y el único endpoint que lo resuelve, `/marketplace/messages/{id}`, responde 403
  // `Invalid caller.id` — pertenece al flujo de aplicaciones *marketplace*, no al de un
  // vendedor común, y el usuario confirmó que la app ya tiene mensajería habilitada. Se
  // descartó además el bloqueo «Model 6» que documenta ML: ése daría 403 también en
  // `/messages/packs/...` y ahí obtenemos 404.
  //
  // Mantener la proyección costaba 39 jobs muertos y 352 reintentos en 7 días, y dejaba la
  // cola de dead-letters permanentemente roja, tapando los fallos que sí importan. El evento
  // se sigue recibiendo y registrando: si algún día se consigue el acceso, sólo hay que volver
  // a enrutar acá. `ingerirMensaje` sigue exportada para el camino directo, pero el job usa
  // `reconciliarMensajesMl`: el id del aviso no es resoluble con credenciales de vendedor.
  const jobType = topic === 'questions' ? 'question.project' : isClaim ? 'claim.project' : esItem ? 'item.project' : 'webhook.audit';
  // `shadow_status='pending'` sólo si la copia de sombra está encendida (`sombra`): con la copia apagada no
  // hay ciclo, y un 'pending' que nadie cierra quedaría activo para siempre y fuera de la purga.
  return db.transaction(() => {
    const inserted = db.prepare(`INSERT OR IGNORE INTO integration_events
      (event_id,event_type,channel,source,external_event_id,resource_id,payload_version,occurred_at,received_at,correlation_id,dedupe_key,metadata_json,status,shadow_status)
      ...
      JSON.stringify({ topic, action, resource: normalizedResource, version, notification_id: notificationId || null, user_id: user, text: envelope.text || null, title: envelope.title || null }), sinJob ? 'completed' : 'pending', sombra ? 'pending' : null);
    if (!inserted.changes) {
      // Es duplicado.
    ...
    db.prepare(`INSERT INTO integration_event_history (event_id,stage,to_status,resource_id,correlation_id,safe_message,created_at) VALUES (?,?,?,?,?,?,?)`)
      .run(eventId, 'webhook.persist', sinJob ? 'completed' : 'pending', normalizedResource, correlationId,
        sinJob ? 'webhook ML de cuenta ajena persistido sólo como traza' : 'webhook ML persistido antes del ACK', receivedAt);
    if (!sinJob) db.prepare(`INSERT INTO integration_jobs (event_id,job_type,available_at) VALUES (?,?,?)`).run(eventId, jobType, receivedAt);
    return { eventId, duplicate: false, ignored: false };
  })();
}

/** Persiste un webhook de producto Woo antes del ACK y lo deduplica por entrega. */
export function registrarWebhookWooProducto(db, envelope, { topic, deliveryId = null, sombra = false } = {}) {
  ...
  return db.transaction(() => {
    const inserted = db.prepare(`INSERT OR IGNORE INTO integration_events
      ...
      'wc-v3', ocurridoEn, receivedAt, correlationId, dedupeKey, JSON.stringify(metadata), 'pending', sombra ? 'pending' : null
    );
    if (!inserted.changes) {
      db.prepare(`INSERT INTO integration_event_history (event_id,stage,to_status,resource_id,correlation_id,safe_message,created_at)
...
/**
 * Persiste un webhook de PEDIDO Woo antes del ACK y lo deduplica por entrega.
 *
 * Es el único de los tres webhooks que no tenía recibo: el handler disparaba `syncPedidoWebPuntual`
 * y `syncWcToMl` con `.catch` y respondía 200, así que un reinicio perdía el aviso sin rastro.
 *
 * NO crea job, y por eso el recibo nace `status='completed'`: el trabajo del legado para un pedido Woo
 * es fail-open y en background (y el cron de 10 min lo repesca). Un `'pending'` sin job quedaría activo
 * para siempre, se leería como cola trabada y la purga de 400 días nunca lo alcanzaría. Lo durable acá
 * es el recibo + la copia de sombra + el barrido, no una cola de reintentos.
 */
export function registrarWebhookWooPedido(db, envelope, { topic = null, deliveryId = null, sombra = false } = {}) {
  const receivedAt = now();
  const normalizedTopic = String(topic || 'order.updated').trim().toLowerCase();
  const pedidoId = Number(envelope?.id);
  if (!Number.isInteger(pedidoId) || pedidoId <= 0) {
    throw Object.assign(new Error('webhook Woo de pedido sin id válido'), { code: 'woo_order_id_invalid', retryable: false });
  }
  const ocurridoEn = envelope?.date_modified_gmt || envelope?.date_modified || envelope?.date_created_gmt || envelope?.date_created || receivedAt;
  const entrega = String(deliveryId || `${pedidoId}:${ocurridoEn}`).trim();
  const fingerprint = `${normalizedTopic}:${pedidoId}:${entrega}`;
  const eventId = `woo-order-webhook-${Buffer.from(fingerprint).toString('base64url')}`;
  const correlationId = `woo-order-${pedidoId}-${eventId}`;
  const recurso = `/orders/${pedidoId}`;
  const metadata = { topic: normalizedTopic, pedido_id: pedidoId, estado: envelope?.status ?? null, delivery_id: deliveryId || null };
  return db.transaction(() => {
    const inserted = db.prepare(`INSERT OR IGNORE INTO integration_events
      (event_id,event_type,channel,source,external_event_id,resource_id,payload_version,occurred_at,received_at,correlation_id,dedupe_key,metadata_json,status,shadow_status)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      eventId, 'webhook.received', 'woo', 'woocommerce', entrega, recurso,
      'wc-v3', ocurridoEn, receivedAt, correlationId, `woo:order-webhook:${fingerprint}`,
      JSON.stringify(metadata), 'completed', sombra ? 'pending' : null
    );
    if (!inserted.changes) {
      db.prepare(`INSERT INTO integration_event_history (event_id,stage,to_status,resource_id,correlation_id,safe_message,created_at)
        VALUES (?,?,?,?,?,?,?)`).run(eventId, 'webhook.dedupe', 'ignored_duplicate', recurso, correlationId,
        'webhook Woo de pedido duplicado ignorado', receivedAt);
      return { eventId, duplicate: true, ignored: true };
    }
    db.prepare(`INSERT INTO integration_event_history (event_id,stage,to_status,resource_id,correlation_id,safe_message,created_at)
      VALUES (?,?,?,?,?,?,?)`).run(eventId, 'webhook.persist', 'completed', recurso, correlationId,
      'webhook Woo de pedido persistido antes del ACK', receivedAt);
    return { eventId, duplicate: false, ignored: false };
  })();
}
  ...
  for (const job of jobs) {
    try {
      ...
      if (job.job_type === 'question.project') {
        ...
        // El aviso trae el id del mensaje pelado, y ese id NO se puede resolver con credenciales
        // de vendedor: `/marketplace/messages/{id}` responde 403 `Invalid caller.id` (es para
        // apps de mensajería) y `/messages/{id}` responde 404. Verificado contra la API el
        // 2026-09-06. Por eso el aviso sólo dispara la reconciliación por pack, que es la vía
        // documentada para un vendedor: el mensaje se trae igual, y de paso se recuperan los que
        // se hayan perdido. Los 39 jobs muertos anteriores ni siquiera llegaban a ese 403 —
        // concatenaban el id a la base y pedían un host inexistente.
        const result = await reconciliarMensajesMl(db, mlCfg, { limite: 5 });
        if (!result?.ok) throw Object.assign(new Error(`MercadoLibre no devolvió los mensajes (${result?.motivo || 'sin motivo'})`), { retryable: true, code: 'ml_message_fetch_failed' });
        ...
        for (const order of orders) {
          ...
          const payload = { status: order.status || 'completed' };
          if (order.tracking) payload.meta_data = [{ key: '_andreani_tracking', value: String(order.tracking) }];
          await wooFetch(wooCfg, `/orders/${order.wc_order_id}`, 'put', payload);
        }
