 ...
 */
export function registrarWebhookMl(db, envelope, { sinJob = false } = {}) {
  ...
  return db.transaction(() => {
    const inserted = db.prepare(`INSERT OR IGNORE INTO integration_events
      ...
      JSON.stringify({ topic, action, resource: normalizedResource, version, notification_id: notificationId || null, user_id: user, text: envelope.text || null, title: envelope.title || null }), sinJob ? 'completed' : 'pending', 'pending');
    if (!inserted.changes) {
      // Es duplicado.
...
export function registrarWebhookWooProducto(db, envelope, { topic, deliveryId = null } = {}) {
  ...
  return db.transaction(() => {
    const inserted = db.prepare(`INSERT OR IGNORE INTO integration_events
      ...
      'wc-v3', ocurridoEn, receivedAt, correlationId, dedupeKey, JSON.stringify(metadata), 'pending', 'pending'
    );
    if (!inserted.changes) {
      db.prepare(`INSERT INTO integration_event_history (event_id,stage,to_status,resource_id,correlation_id,safe_message,created_at)
 ...
 */
export function registrarWebhookWooPedido(db, envelope, { topic = null, deliveryId = null } = {}) {
  const receivedAt = now();
  ...
  return db.transaction(() => {
    const inserted = db.prepare(`INSERT OR IGNORE INTO integration_events
      ...
      JSON.stringify(metadata), 'completed', 'pending'
    );
    if (!inserted.changes) {
      db.prepare(`INSERT INTO integration_event_history (event_id,stage,to_status,resource_id,correlation_id,safe_message,created_at)
