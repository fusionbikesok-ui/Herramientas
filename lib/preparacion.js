...
// ¿El paquete ya salió del punto de despacho? ML a veces deja `status='ready_to_ship'`
// desactualizado mientras `substatus` ya avanzó a que el paquete se entregó/retiró —
// incidente real: pack 2000015175820985 (orden 2000018610145024, xd_drop_off) siguió en la
// cola de Preparación con status ready_to_ship y substatus dropped_off. `shipped`/`delivered`
// entran por `status` directo (esos SÍ son confiables). No se toca el significado de
// 'cancelled': eso lo decide clasificarElegibilidadMl/estadoComercial por su cuenta.
const ENVIOS_ML_YA_SALIO = new Set(['shipped', 'delivered']);
const SUBSTATUS_ML_YA_SALIO = new Set(['dropped_off', 'picked_up', 'in_hub', 'authorized_by_carrier']);
export function envioMlYaSalio(envio) {
  if (!envio?.status) return false;
  if (ENVIOS_ML_YA_SALIO.has(envio.status)) return true;
  return envio.status === 'ready_to_ship' && SUBSTATUS_ML_YA_SALIO.has(envio.substatus);
}
...
export function clasificarElegibilidadMl(orden, envio = null) {
  ...
  if (envioMlYaSalio(envio)) return { estado: 'no_elegible', motivo: 'envio_ya_salio' };
...
// Elegibilidad y prioridad de la cola continua de preparación. No congela lotes ni olas:
// cada pedido nuevo aparece en la próxima actualización y se prepara directamente.
export function pedidosElegiblesOrdenados(db) {
  const tienePreparaciones = db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='preparaciones'").get();
  const filtroLocal = tienePreparaciones
    ? "AND NOT EXISTS (SELECT 1 FROM preparaciones p WHERE p.clave=pedidos_cache.clave AND p.estado IN ('completada', 'pendiente_deposito', 'cerrada_sin_evidencia', 'despachada_sin_verificar'))"
    : '';
  return db.prepare(
    `SELECT * FROM pedidos_cache WHERE estado_envio='pendiente' ${filtroLocal}
     ORDER BY CASE WHEN canal='ml' OR espejo_ml=1 THEN 0 ELSE 1 END, fecha ASC`
  ).all();
}
