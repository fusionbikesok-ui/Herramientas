 ...
 *    en vez de cortar en ~1,4s. Por eso se rastrea `real429EstaCorrida` por corrida: el primer
 *    429 sin marcas la enciende, y un `__cooldownSintetico` posterior EN LA MISMA CORRIDA se
 *    trata como continuación de ese 429 real (consume REINTENTOS_429_MS, termina en
 *    `cortado_por_rate_limit`). Solo un `__cooldownSintetico` que aparece SIN que esta corrida
 *    haya visto un 429 real antes —heredado de otro cron de los 9 corriendo en paralelo— va a
 *    la rama de espera (`cortado_por_cooldown_propio`). `__sinCupo` no participa de esta
 *    distinción: es presupuesto propio (mlRateLimiter.js), no una respuesta de ML, así que
 *    siempre va a la rama de espera, acotada por MAX_REINTENTOS_SIN_CUPO (ver esa constante).
 *    El estado sondeable (`getEstadoPush`) distingue `cortado_por_rate_limit` (ML real) de
 *    `cortado_por_cooldown_propio` (nuestro) — antes ambos caían en el mismo booleano.
...
export async function pushSkusPendientes(db, cfg, { limite = LOTE_PUSH, cuotaPausadas = CUOTA_PAUSADAS_DEFAULT } = {}) {
  ...
  try {
    ...
    // IMPORTANTE 1 / BLOQUEANTE 1 (revisor): true desde el primer 429 SIN marcas que vea esta
    // corrida — mlFetch activa su propio cooldown antes de devolvérnoslo, así que el intento
    // siguiente de la misma publicación llega acá como __cooldownSintetico aunque el 429 que
    // lo originó fue real. Un sintético posterior con esta bandera en true es continuación de
    // ESE 429 real (ver bloque de manejo de 429 más abajo), no un cooldown heredado de otro cron.
    let real429EstaCorrida = false;
    ...
    while (true) {
      ...
      for (const p of lote) {
        ...
        if (Date.now() - inicioCorrida > TIEMPO_MAX_CORRIDA_MS) { cortadoCooldownPropio = true; break; }
        ...
        for (let intentoReal = 0; ; ) {
          ...
          if (resultado.__cooldownSintetico && !real429EstaCorrida) {
          ...
          if (!resultado.__cooldownSintetico) real429EstaCorrida = true;
          if (intentoReal >= REINTENTOS_429_MS.length) { cortadoRateLimit = true; break; }
          await sleep(REINTENTOS_429_MS[intentoReal]);
          intentoReal++;
        }
        if (cortadoRateLimit || cortadoCooldownPropio) break; // resto de la corrida queda para el próximo ciclo, sin marcar fallo
      ...
      if (cortadoRateLimit || cortadoCooldownPropio || cortadoError) break;
    }
