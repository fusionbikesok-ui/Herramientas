 ...
 *      · Sin esas marcas — 429 REAL de ML: comportamiento intacto, reintentos con
 *        REINTENTOS_429_MS y corte de la corrida entera sin marcar fallo si persiste.
    ...
    while (true) {
      ...
      for (const p of lote) {
        ...
        for (let intentoReal = 0; ; ) {
          ...
          if (resultado.__cooldownSintetico || resultado.__sinCupo) {
            // 429 NUESTRO (cooldown propio o presupuesto propio agotado): ML no fue
            // consultado. No cuenta como fallo de la publicación ni consume los reintentos
            // de un 429 real — se espera a que pase y se reintenta la MISMA publicación.
            let esperaMs = ESPERA_SIN_CUPO_MS;
            if (resultado.__cooldownSintetico) {
              const { hasta } = estadoCooldownMl();
              esperaMs = hasta ? Math.max(50, new Date(hasta).getTime() - Date.now()) : ESPERA_SIN_CUPO_MS;
            }
          ...
          // 429 REAL de ML (sin marcas nuestras): comportamiento intacto.
