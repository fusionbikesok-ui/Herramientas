...
import {
  ...
  diffTokens, confianzaDesdeScore, otrasMarcasPosibles, esDiscriminante, esCodigoAlfanumerico, TOPE_CANDIDATOS,
...
export function candidatosParaDoc(doc, wcIndex) {
  ...
  // DEFECTO B1 (piloto Pedalar #205, José): códigos de modelo alfanuméricos que declara el
  // documento (hr70, bsc500, tl50 — mezcla letra+dígito, regla 1 de esDiscriminante). Un
  // candidato que comparte el código nunca puede quedar, en el orden, por debajo de uno cuyo
  // código está en conflicto (hr40 cuando el documento dice hr70) — eso ya lo topea la
  // confianza (el código ausente del candidato es discriminante y activa hay_contradiccion,
  // ver diffTokens), pero dentro de la MISMA confianza el score de bolsa de palabras podía
  // dejar arriba al que NO comparte el código solo por compartir más relleno textual. Es un
  // criterio de ORDEN explícito, no un peso adicional al score.
  const codigosDoc = [...tks].filter((t) => esCodigoAlfanumerico(t));
  ...
  for (const pos of candPos) {
    ...
    const codigoModeloCompartido = codigosDoc.length > 0 && codigosDoc.some((c) => wcToks.has(c));
    scored.push({
      score: +sm.toFixed(3), confianza, diff, attrBonus: attrScore(cOk) + attrScore(tOk),
      contradiccion_atributo: contra.hay, color_ok: cOk, talle_ok: tOk, codigo_modelo_compartido: codigoModeloCompartido,
  ...
  scored.sort((a, b) => (a.filtrado_por_atributo - b.filtrado_por_atributo)
    ...
    || (b.codigo_modelo_compartido - a.codigo_modelo_compartido) // B1: código de modelo antes que el score de relleno
  ...
  // candidato del MISMO nivel de confianza (uno 'baja' no le disputa nada a uno 'alta').
  //
  // Dos casos, con criterios distintos a propósito:
  //  - HERMANOS (mismo id_padre): siempre ambiguo, sin epsilon. Sus scores son casi idénticos
  //    por definición (comparten el título base), así que un umbral ahí es arbitrario: lo que
  //    decide entre un talle y otro no es el score. Si el documento SÍ trae el talle, los
  //    hermanos que lo contradicen ya salieron de `enJuego` más arriba y este bloque ni se
  //    evalúa — el camino feliz sigue siendo automático.
  //  - NO HERMANOS: ambiguo si empatan a menos de EPSILON_EMPATE. Por debajo de ese margen el
  //    score no está eligiendo, está desempatando por orden de catálogo, que es exactamente el
  //    bug que este módulo corrige. Se compara `id_woo` porque dos productos DISTINTOS pueden
  //    tener título idéntico (pasa en el catálogo real).
  //
  // OJO (dependencia del caller): esto se apoya en que `id_woo` venga poblado. Si la consulta
  // que arma el índice no lo trae, todos quedan null, la comparación da false y la protección
  // se apaga en silencio. La query del endpoint de ingreso DEBE seleccionar id_woo.
  let ambiguo = false;
  const [c1, c2] = enJuego;
  if (c1 && c2 && RANGO[c1.confianza] === RANGO[c2.confianza]) {
    ...
    if (sonHermanos) {
      ...
      ambiguo = Math.abs(c1.score - c2.score) < EPSILON_EMPATE && c1.id_woo !== c2.id_woo;
    }
  }
