...
export function candidatosParaDoc(doc, wcIndex) {
  ...
  for (const pos of candPos) {
    ...
    const contra = contradiccionAtributo(ctDoc, wc);
  ...
  scored.sort((a, b) => (a.filtrado_por_atributo - b.filtrado_por_atributo)
    || (RANGO[b.confianza] - RANGO[a.confianza]) || (b.score - a.score) || (b.attrBonus - a.attrBonus));
  ...
  let ambiguo = false;
  const [c1, c2] = enJuego;
  if (c1 && c2 && RANGO[c1.confianza] === RANGO[c2.confianza] && Math.abs(c1.score - c2.score) < EPSILON_EMPATE) {
    ambiguo = difierenEnAtributo(c1, c2);
  }
