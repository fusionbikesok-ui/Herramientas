...
function refreshActivo(db, raw, userId) {
  ensureMobileAuthTables(db);
  const row = db.prepare('SELECT * FROM mobile_refresh_tokens WHERE token_hash = ? AND user_id = ?').get(tokenHash(raw), userId);
  return row && !row.revocado_en && !row.usado_en && new Date(row.expires_at) > new Date() ? row : null;
}
...
export function validarRefreshToken(db, raw, userId) { return Boolean(refreshActivo(db, raw, userId)); }

export function asociarRefreshConDispositivo(db, raw, userId, deviceId) {
  const result = db.prepare(`UPDATE mobile_refresh_tokens SET device_id = ?
    WHERE token_hash = ? AND user_id = ? AND revocado_en IS NULL AND usado_en IS NULL`).run(deviceId, tokenHash(raw), userId);
  return result.changes === 1;
}

export function revocarRefreshPorDispositivo(db, deviceId, userId) {
  const result = db.prepare(`UPDATE mobile_refresh_tokens SET revocado_en = COALESCE(revocado_en, ?)
    WHERE device_id = ? AND user_id = ?`).run(new Date().toISOString(), deviceId, userId);
