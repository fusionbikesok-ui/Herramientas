...
export function buscarAliasVigente(db, input) {
  ...
  if (!row || !db.prepare("SELECT 1 FROM catalogo_cache WHERE id_woo=? AND tipo IN ('simple','variation')").get(row.id_woo)) return null;
  return row;
}
...
export function confirmarAlias(db, input) {
  ...
  const productoValido = db.prepare("SELECT 1 FROM catalogo_cache WHERE id_woo=? AND tipo IN ('simple','variation')").get(input.id_woo);
  if (!productoValido) throw new Error('id_woo no corresponde a un producto vendible en catalogo_cache');
