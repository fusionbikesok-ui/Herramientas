...
export async function barridoAuditoria(db, mlCfg) {
  ...
  for (let i = 0; i < itemIds.length; i += CHUNK) {
    ...
    const resp = await mlFetch(db, mlCfg, 'get',
      `/items/bulk?ids=${chunk.join(',')}&attributes=status_code,id,body.id,body.health,body.pictures,body.video_id`
    );
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare("INSERT OR REPLACE INTO sync_estado (clave, valor, actualizado_en) VALUES ('cursor_auditoria', ?, ?)")
      .run(ultimaClave, now);
  });
  tx();
