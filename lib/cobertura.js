...
/**
 * Decide si un producto tiene "demasiadas" publicaciones ML para el mismo SKU
 * (más de `umbral`), es decir, un caso a revisar por multi-publicación.
 *
 * A diferencia de esFaltante, NO aplica el filtro de stock > 0: el chequeo vale
 * sin importar el stock. Sí aplica las demás exclusiones (variable, sin SKU,
 * no-vendible, solo local).
 *
 * @param {object}  prod          fila de catalogo_cache: { id_woo, sku, tipo, categorias_json }
 * @param {Map<string, number>} conteoPorSku  SKU → cantidad de publicaciones ML con ese seller_sku
 * @param {Set<number>} excluidos  id_woo marcados manualmente como "solo local"
 * @param {number}  umbral        cantidad máxima aceptable (por defecto 2)
 * @returns {boolean}
 */
export function esMultiPublicacion(prod, conteoPorSku, excluidos, umbral = 2) {
  if (prod.tipo === 'variable') return false;           // padre variable: placeholder
  const sku = String(prod.sku || '').trim();
  if (!sku) return false;                               // sin SKU no se puede mapear
  if (excluidos && excluidos.has(prod.id_woo)) return false; // solo local
  if (esNoVendible(prod)) return false;                 // servicio / QR pagos
  const conteo = (conteoPorSku && conteoPorSku.get(sku)) || 0;
  return conteo > umbral;
}

/**
 * Filtra un catálogo completo devolviendo solo los casos de multi-publicación.
 */
export function calcularMultiPublicacion(catalogo, conteoPorSku, excluidos, umbral = 2) {
  return catalogo.filter((p) => esMultiPublicacion(p, conteoPorSku, excluidos, umbral));
}

/**
 * true si el status de una publicación ML cuenta como "activa".
 * Espejo exacto del criterio del cliente: vacío / 'active' / 'activa' → activa;
 * cualquier otro estado (paused, etc.) se considera pausada/inactiva.
 */
export function esActivaMl(status) {
  const st = String(status || '').toLowerCase();
  return !st || st === 'active' || st === 'activa';
}
