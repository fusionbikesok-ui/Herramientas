...
export function esClaveCubierta(db, clave) {
  ...
  // 3. Si hay seller_sku en ml_publicaciones_cache, debe coincidir EXACTAMENTE con el SKU decidido
  //    (si no hay registro remoto, asumimos que está cubierto localmente por la decisión verificada)
  ...
  // (pero solo si existe registro remoto — si no existe, confiar en la decisión local)
  const remoto = db.prepare('SELECT seller_sku FROM ml_publicaciones_cache WHERE clave=?').get(clave);
  if (!remoto) {
    // Sin registro remoto en caché, la decisión local es suficiente (ya fue verificada por una persona)
    return true;
  }
