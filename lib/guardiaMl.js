...
export function esClaveCubierta(db, clave) {
  return !!db.prepare(`SELECT 1 FROM sku_matcher_decisiones d JOIN catalogo_cache c ON c.sku=d.sku
    WHERE d.clave=? AND d.accion IN ${VALIDAS} AND trim(COALESCE(d.sku,''))<>'' LIMIT 1`).get(clave);
}
...
export function escanearGuardiaMl(db, actor = 'sistema', { lecturaMlConfirmada = false } = {}) {
  ...
  if (modo === 'acciones') {
    ...
    db.transaction(() => { for (const c of candidatas) {
      const otra = db.prepare("SELECT 1 FROM sku_matcher_decisiones WHERE sku=? AND accion IN ('asignar','confirmar') LIMIT 1").get(c.sku);
      const compartido = db.prepare('SELECT 1 FROM guardia_ml_stock_compartido WHERE sku=?').get(c.sku);
      if (!otra && !compartido && insertarAuto.run(c.clave,c.sku,c.nombre,ts).changes) {
  ...
  const filas = db.prepare(`SELECT p.clave FROM ml_publicaciones_cache p
    WHERE p.status='active' AND COALESCE(p.available_quantity,0)>0
    AND NOT EXISTS (SELECT 1 FROM sku_matcher_decisiones d JOIN catalogo_cache c ON c.sku=d.sku
      WHERE d.clave=p.clave AND d.accion IN ${VALIDAS} AND trim(COALESCE(d.sku,''))<>'')`).all();
    ...
    try {
      ...
      if (op.tipo === 'vincular') {
        ...
        if (otrasClaves > 0 && !db.prepare('SELECT 1 FROM guardia_ml_stock_compartido WHERE sku=?').get(op.sku)) {
          throw new Error('requiere confirmación de stock compartido antes de escribir en ML');
        }
        const previo = db.prepare('SELECT seller_sku FROM ml_publicaciones_cache WHERE clave=?').get(caso?.clave)?.seller_sku || '';
        const decision = db.prepare("SELECT sku FROM sku_matcher_decisiones WHERE clave=? AND accion IN ('asignar','confirmar') ORDER BY id DESC LIMIT 1").get(caso?.clave)?.sku || '';
        if ((previo && previo !== op.sku) || (decision && decision !== op.sku)) {
          const r = await desvincularSkuEnMl(db,cfg?.ml||cfg,caso?.clave);
        ...
        resultado = await escribirSkuEnMl(db,cfg?.ml||cfg,caso?.clave,op.sku,{manual:true});
      } else resultado = await pausarPublicacionMl(db,cfg?.ml||cfg,op.item_id);
