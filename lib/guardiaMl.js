...
import { INTEGRACION_GUARDIA_ML, PROCESO_VENTA_RETENIDA } from './guardiaAvisos.js';
...
export function retenerPedidoMl(db, { orderId, items, claves, motivo = 'sin_cobertura' }) {
  ...
  abrirAvisoVentaRetenida(db, { orderId, items, claves, motivo });
}

// Una alerta por venta (tipoError = order id), severidad advertencia: el push lo decide
// lib/workerNotificacionesPush.js (quienes pueden liberar, un recordatorio a las 2 h) y no dispara
// email. Fail-open: `abrirOActualizarIncidente` nunca lanza.
function abrirAvisoVentaRetenida(db, { orderId, items, claves, motivo }) {
  const primero = (items || [])[0]?.item?.title || (items || [])[0]?.titulo || null;
  const producto = primero ? `"${String(primero).slice(0, 80)}"` : `la venta ${orderId}`;
  const extra = (items || []).length > 1 ? ` y ${(items || []).length - 1} producto(s) más` : '';
  abrirOActualizarIncidente(db, {
    integracion: INTEGRACION_GUARDIA_ML,
    proceso: PROCESO_VENTA_RETENIDA,
    tipoError: String(orderId),
    severidad: 'advertencia',
    mensajeHumano: `Venta pagada retenida: ${producto}${extra} no llega a Woo hasta vincular el producto o liberarla en Guardia ML.`,
    mensajeTecnico: `pedido ${orderId} retenido (${motivo}); claves: ${(claves || []).join(', ')}`,
    contexto: { ml_order_id: String(orderId), motivo, claves: claves || [] },
  });
}

export function cerrarAvisoVentaRetenida(db, orderId) {
  return confirmarCicloSano(db, { integracion: INTEGRACION_GUARDIA_ML, proceso: PROCESO_VENTA_RETENIDA, tipoError: String(orderId) });
}

// Única implementación de "liberar": la usan el endpoint manual y la liberación automática.
// Deja la venta lista para que la próxima importación cree el pedido Woo.
export function liberarPedidoRetenido(db, orderId, { actor, motivo }) {
  const id = String(orderId);
  let ok = false;
  db.transaction(() => {
    const ts = now();
    const actualizado = db.prepare("UPDATE guardia_ml_pedidos_retenidos SET estado='liberado', responsable=?, liberado_en=?, liberado_por=?, actualizado_en=? WHERE ml_order_id=? AND estado='retenido'")
      .run(actor, ts, actor, ts, id);
    if (!actualizado.changes) return;
    db.prepare('DELETE FROM ordenes_ml_wc_pedidos WHERE ml_order_id=? AND wc_order_id=0 AND retenido_en IS NULL').run(id);
    db.prepare('DELETE FROM ordenes_ml_procesadas WHERE order_id=? AND NOT EXISTS (SELECT 1 FROM ordenes_ml_wc_pedidos WHERE ml_order_id=? AND wc_order_id<>0)').run(id, id);
    const caso = db.prepare("SELECT id FROM guardia_ml_casos WHERE pedido_ml_order_id=? AND estado!='resuelto' ORDER BY id DESC LIMIT 1").get(id);
    if (caso) registrarEventoGuardia(db, caso.id, 'pedido_liberado', actor, { order_id: id, motivo });
    ok = true;
  })();
  if (ok) cerrarAvisoVentaRetenida(db, id);
  return ok;
}

// Claves `item_id|variation_id` de una venta retenida, con el mismo formato que normalizarOrdenMl.
// Devuelve [] si algún ítem no trae id: sin claves conocidas no se libera nada automáticamente.
export function clavesDePedidoRetenido(fila) {
  let items;
  try { items = JSON.parse(fila?.items_json || '[]'); } catch { return []; }
  if (!Array.isArray(items) || !items.length) return [];
  const claves = [];
  for (const oi of items) {
    const itemId = oi?.item?.id;
    if (!itemId) return [];
    claves.push({ clave: armarClaveMl(itemId, oi.item.variation_id), sellerSku: String(oi.item.seller_sku || '').trim() });
  }
  return claves;
}

// Regla compartida con syncMlToWc (routes/sync.js): un seller_sku cubre la venta sólo si resuelve a
// EXACTAMENTE un producto del catálogo Woo. Única implementación: la importan ambos lados.
export function skuUnicoEnCatalogo(db, sku) {
  const limpio = String(sku ?? '').trim();
  if (!limpio) return false;
  return db.prepare('SELECT COUNT(*) AS n FROM catalogo_cache WHERE sku = ?').get(limpio).n === 1;
}

// Libera las ventas retenidas cuyo bloqueo ya se resolvió: TODAS sus claves cubiertas (vínculo
// exacto, o seller_sku de la venta que resuelve a un único SKU — la misma regla con la que
// syncMlToWc decide retener) y ninguna bloqueada por Guardia. Una excepción no libera: el caso sigue
// bloqueando y sin producto vinculado no hay pedido Woo que crear. Fail-open por venta.
export function liberarRetenidasResueltas(db) {
  let liberadas = 0;
  let revisadas = 0;
  const retenidas = db.prepare("SELECT ml_order_id, items_json FROM guardia_ml_pedidos_retenidos WHERE estado='retenido' ORDER BY creado_en").all();
  for (const fila of retenidas) {
    revisadas++;
    try {
      const claves = clavesDePedidoRetenido(fila);
      if (!claves.length) continue;
      const resueltas = claves.every(({ clave, sellerSku }) =>
        (esClaveCubierta(db, clave) || skuUnicoEnCatalogo(db, sellerSku)) && !claveBloqueadaGuardia(db, clave));
      if (!resueltas) continue;
      const motivo = `bloqueo resuelto: ${claves.map(c => c.clave).join(', ')}`;
      if (liberarPedidoRetenido(db, fila.ml_order_id, { actor: 'sistema', motivo })) liberadas++;
    } catch (e) {
      console.error('[guardia-ml] no se pudo evaluar la venta retenida', fila.ml_order_id, e.message);
    }
  }
...
export function resolverRetencionPedidoMl(db, orderId, actor, estado = 'liberado') {
  ...
  if (result.changes > 0) cerrarAvisoVentaRetenida(db, orderId);
...
export async function procesarOperacionesGuardia(db, cfg) {
  ...
  for (const op of rows) {
    ...
    try {
      const casoActual = db.prepare("SELECT estado,responsable,expected_version,clave FROM guardia_ml_casos WHERE id=?").get(op.caso_id);
      const estadoCaso = casoActual?.estado;
      // Un solo escritor por clave (UM1.6). `POST /vincular-clave` ya delega en Identidad
      // cuando ésta gobierna la clave, pero pueden quedar operaciones encoladas de antes de
      // esa delegación: no se ejecutan. Escribir acá pondría a dos schedulers sobre la misma
      // publicación, que es exactamente lo que produjo las 26 operaciones en `conflicto`.
      if (casoActual?.clave) {
        const gobernadaPorUm1 = db.prepare("SELECT 1 FROM identidad_casos WHERE direccion='ml_fusion' AND ml_key=? AND estado<>'resuelto'").get(casoActual.clave);
        if (gobernadaPorUm1) throw new Error('la clave la gobierna Identidad de productos; la escritura sale por su worker');
      }
    ...
    const ts=now();
    // `la gobierna Identidad` va acá y no al camino de reintento: no es un fallo transitorio,
    // es que esta operación ya no le corresponde a Guardia. Reintentarla es gastar ciclos
    // para volver a chocar contra lo mismo.
    if (!resultado?.ok && /caso ya no está pendiente|versión del caso cambió|responsable del caso cambió|la gobierna Identidad de productos/.test(String(resultado?.error||''))) {
...
/**
 * Cierra los casos de Guardia que nunca debieron abrirse: los de publicaciones que no se venden
 * por el marketplace.
 *
 * Guardia mira `status='active' AND available_quantity>0` sin filtrar canal, así que levanta los
 * links de pago de Mercado Pago (`mp-merchants`, `mp-link`) como si fueran publicaciones. Medido
 * el 2026-09-06: su universo es de 1180 publicaciones contra las 1072 de UM1, y esas 108 de
 * diferencia son todas links de pago. Los 67 casos abiertos que quedaban eran, uno por uno, de
 * ese tipo.
 *
 * No es sólo ruido: las 26 operaciones que quedaron en conflicto eran de tipo `pausar` sobre esos
 * mismos links. Si alguien tomaba uno de esos casos, Guardia pausaba un link de pago activo.
 *
 * Cierra y cancela; no borra. El historial de Guardia es la evidencia de por qué se retiró.
 */
export function cerrarCasosFueraDeUniverso(db, actor = 'sistema') {
  const ts = now();
  const casos = db.prepare(`SELECT c.id, c.clave FROM guardia_ml_casos c
    JOIN ml_publicaciones_cache m ON m.clave = c.clave
   WHERE c.estado IN ('abierto','tomado','excepcion')
     AND m.canales_json IS NOT NULL AND m.canales_json NOT LIKE '%marketplace%'`).all();
  // No hay salida temprana por `casos.length === 0`: una operación huérfana puede seguir viva
  // sin ningún caso abierto que la acompañe, y saltearla dejaría un `pausar` armado sobre un
  // link de pago. Lo detectó un test antes de que pasara.
  let operaciones = 0;
  db.transaction(() => {
    // Primero las operaciones huérfanas: una `pausar` puede seguir viva sobre una publicación
    // fuera de universo aunque su caso ya esté resuelto, y entonces el filtro por estado del
    // caso no la alcanza. Se encontró una así, apuntando a un link de pago con stock.
    operaciones += db.prepare(`UPDATE guardia_ml_operaciones
      SET estado='cancelada', ultimo_error='operación fuera del universo: no se vende por marketplace', actualizado_en=?
      WHERE estado IN ('pendiente','conflicto','procesando')
        AND item_id IN (SELECT substr(m.clave, 1, instr(m.clave, '|') - 1) FROM ml_publicaciones_cache m
                         WHERE m.canales_json IS NOT NULL AND m.canales_json NOT LIKE '%marketplace%')`).run(ts).changes;
    for (const c of casos) {
      // Las operaciones se cancelan ANTES de cerrar el caso: si el worker tomara una entre
      // ambos pasos, ejecutaría un `pausar` sobre un caso que ya no existe como problema.
      const ops = db.prepare(`UPDATE guardia_ml_operaciones
        SET estado='cancelada', ultimo_error='caso fuera del universo: no se vende por marketplace', actualizado_en=?
        WHERE caso_id=? AND estado IN ('pendiente','conflicto','procesando')`).run(ts, c.id);
      operaciones += ops.changes;
      db.prepare(`UPDATE guardia_ml_casos SET estado='resuelto', resuelto_en=?, actualizado_en=?,
        expected_version=expected_version+1 WHERE id=?`).run(ts, ts, c.id);
      registrarEventoGuardia(db, c.id, 'cerrado_fuera_de_universo', actor, {
        clave: c.clave, motivo: 'link de pago de Mercado Pago; no se vende por el marketplace',
        operaciones_canceladas: ops.changes,
      });
    }
  })();
  return { casos: casos.length, operaciones };
}
