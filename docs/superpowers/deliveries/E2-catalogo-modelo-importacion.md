...
**Estado:** tramo 1 (modelos y variantes) desplegado en producción el 2026-09-20; **tramo 2 (atributos, imágenes y datos comerciales) desplegado en producción el 2026-09-20** (migración `0014`, worker recreado, backfill de las 12.850 representaciones ejecutado; la comparación entre canales se encendió el mismo día, `CATALOGO_COMPARAR_ATRIBUTOS=1`); tramo 3 (taxonomía propia, colecciones, packs y kits) **con plan escrito** en `docs/superpowers/plans/2026-09-20-e2-tramo3-taxonomia-colecciones-packs.md`, sin implementar.
...
- **Próxima acción exacta:** decidir con José si se enciende `CATALOGO_COMPARAR_ATRIBUTOS=1`. El tramo 2 ya está desplegado y capturando; el ensayo en seco dice que la comparación abriría **114 casos** `atributo_divergente`, entre ellos dos errores de carga reales (`amarilllo`, `sporatace`). La variable ya está declarada en el bloque `environment` del worker, así que basta agregarla a `plataforma.env` y recrear el worker.
- **Tramo 2 implementado y desplegado (2026-09-20).** Cinco tareas, delegadas a la sesión `opt-2b` y revisadas por mutación una por una: migración `0014` (7 columnas en `external_representations`, `catalog.model_attributes`, `catalog.model_images`, tipo de caso `atributo_divergente`); extracción pura de atributos, imágenes y datos comerciales de los payloads que el proyector ya recibía y descartaba; persistencia en la misma transacción del mensaje, con `vigente_hasta` en vez de borrar y el caso de divergencia colgado de la representación; backfill de las 12.849 representaciones existentes desde los cachés del legado, por lotes con checkpoint `capturado_en IS NULL` y sin tabla nueva; y la puesta al día del contrato `schema.sql`. **Suites: plataforma 495/495, legado 2.755 (51 skipped), cero fallos.** Tres defectos aparecieron y ninguno llegó al VPS: el contrato del esquema desactualizado (lo atrapó la suite completa, no una revisión), la coma que es también separador decimal (lo atrapó una medición sobre los datos reales) y `openDb` que corre migraciones al abrir, o sea que un script de sólo lectura habría escrito en la base de producción del legado (lo atrapó `opt-2b`).
- **Despliegue del tramo 2 (2026-09-20).** Migración `0014` aplicada, worker recreado, backfill ejecutado: **12.850 representaciones capturadas, 0 pendientes**, 86.632 atributos y 10.482 imágenes. Apareció un cuarto defecto, antes de desplegar: **`CATALOGO_COMPARAR_ATRIBUTOS` no estaba en `compose.yml`**, o sea que la bandera era inencendible desde el host — el mismo hueco que dejó el catálogo sin proyectar el 19. Tercera repetición de la clase, así que ahora la vigila `E2-CFG-03`. **El resumen del backfill cuenta intenciones y no filas guardadas**: la diferencia contra el ensayo en seco son las 2.224 representaciones `omitida_por_decision`, que guardan el crudo pero no atributos derivados porque no cuelgan de ningún modelo. Ningún dato se perdió.
- **Alcance del tramo 2 que NO entró, a propósito:** unificar sinónimos de atributos (`gris`/`stone gray`, `plata`/`plateado`) pide un diccionario y es decisión de negocio; ML no parte sus valores multivalor porque su coma es genuinamente ambigua (~722 valores quedan como string opaco, con el crudo guardado para reproyectar); y el backfill deja una sola imagen por representación porque el caché del legado guarda una — el resto llega cuando el proyector vuelva a observar el producto.
- **Definir con José el alcance del tramo 3** — no hay diseño escrito más allá del tramo 1. El candidato natural son los **~4.700 casos de identidad abiertos** que el bootstrap destapó (2.227 `omitida_revisar`, 1.991 `sku_pendiente`, 422 `user_product_divergente`, 49 `woo_sku_no_canonico`, 17 `sku_inexistente_en_woo`, 14 `identidad_legado`): son decisiones de negocio pendientes, no fallas, y caen en el territorio de E3 (identidad y matcher único en sombra), así que hay que resolver si se atienden acá o allá.
