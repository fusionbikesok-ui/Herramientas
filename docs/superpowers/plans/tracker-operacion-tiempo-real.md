...
| **A.4** — Reclamos sumados a "Novedades ML" | ⬜ **Sin empezar** | — | Extender `routes/notificacionesMl.js` con el topic `claims`, mismo patrón que `questions`/`messages`: tabla nueva (`ml_reclamos` o similar), `ingerirReclamo(db, mlCfg, resource)` fail-open, sumar a `/count`, `/pendientes` y el aviso del Home. Agregar `if (topic === 'claims')` en `server.js`. **Migración: la `019` YA NO está disponible** (se usó para `incidentes_operativos` del frente de confiabilidad operativa, 2026-08-27/28 — ver `docs/api-contrato.md`) — usar la próxima migración libre en `migrations/` al momento de implementar A.4, no la 019. |
