...
| **A.4** — Reclamos sumados a "Novedades ML" | ✅ **Integrado y E2E aprobado localmente** | `hito-a4-claims` → `c4263d4` → fast-forward a `conteo-confiable` (2026-08-28) | Incluye `migrations/027_ml_reclamos.sql` (sin colisión con Hito 7), tabla defensiva, `ingerirReclamo` fail-open ante errores de ML, topic `claims`, reclamos en `/count` y `/pendientes`, banner Home y contrato actualizado. Validación dirigida final: 2 archivos, 19/19 tests verdes (`test/notificacionesMl.test.js` + `test/server.test.js`); `git diff --check` OK. E2E aislado con usuario de prueba y reclamo sembrado: banner correcto en 1440/768/390, sin overflow; solo 404 de recurso auxiliar no relacionado. Revisión estática sin hallazgos bloqueantes. La suite global previa tuvo fallos ajenos en inventario/sync/matcher/reactivación/reconciliación y no se usa como aprobación de esos módulos. Deploy y push siguen siendo manuales. |
