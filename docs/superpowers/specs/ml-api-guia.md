...
| `/items?ids=` y `/users?ids=` — deprecación | En proceso de deprecación. Migrar a **`/items/bulk?ids=`** y **`/users/bulk?ids=`** antes del **25/10/2026**. Durante la convivencia ambos endpoints funcionan. Multiget sigue siendo **máx. 20 ids** por llamada. `attributes=` filtra campos del `body` de cada resultado (`attributes=body.x`). **`/items/bulk` cambia la forma de la respuesta respecto de `/items?ids=`: el campo `code` pasa a llamarse `status_code`, y la respuesta incluye `id` en el nivel raíz de cada elemento** — el código que migra debe leer `status_code` (o aceptar ambos campos durante la convivencia), no asumir que la forma es idéntica. **Ojo — comportamiento observado, no documentado en la página oficial (verificado 2026-09-25 con 5 GET reales de solo lectura contra la API):** el filtro `attributes=` se aplica al elemento *entero* de la respuesta, no sólo al `body`. Sin `attributes=`, cada elemento trae `{id, status_code, body}`; pero con `attributes=body.x,...` el elemento queda **sólo** `{body: {...filtrado...}}` — sin `id` ni `status_code` en la raíz. Para conservar ambos hay que pedirlos explícitamente: `attributes=status_code,id,body.x,body.y,...`. Un código que filtra por `status_code` (o `code`) y usa `attributes=body.*` sin agregar `status_code,id,` descarta **todos** los elementos silenciosamente. `include_attributes=all` sí funciona igual en `/items/bulk` que en el endpoint viejo (probado con `attributes=status_code,id,body.id,body.status,body.attributes`: devuelve `body.attributes` completo). | `items-y-busquedas.txt:7,69-72,726-733`; comportamiento del filtro `attributes=` sobre el elemento raíz: sonda propia `sonda-bulk.mjs` (`/root/backups-worker/ml-investigacion-20260925/`), no documentado en la página oficial |
...
| `items` | Multiget (`/items/bulk?ids=`, máx. 20, `attributes=` acotado) para lecturas en lote; evento (`items`) para cambios puntuales. Evitar `/items?ids=` en código nuevo desde ya. **Ojo:** `/items/bulk` responde `{id, status_code, body}` por elemento (no `{code, body}`); el parseo debe leer `status_code`. **Si además usás `attributes=`, agregá `status_code,id,` al principio de la lista** (`attributes=status_code,id,body.x,...`) — de lo contrario el elemento pierde `id` y `status_code` (ver §1). | Evento + barrido de catálogo espaciado. |
...
- La rama `fix/legado-ml-mitigacion-temporal` migró los usos legado de `/items?ids=` a `/items/bulk?ids=` (adaptando el parseo a `status_code`, aceptando también `code` por compatibilidad durante la convivencia) y agregó `x-format-new: true` a las lecturas legado de `/shipments/{id}` que no lo tenían (`routes/sync.js`, `routes/preparacion.js`). No toca el gateway sombra ni su cupo. Sus `attributes=` llevan el prefijo `status_code,id,body.` en los 10 sitios que filtran (ver §1) — descubierto tras un primer intento que usó sólo `attributes=body.x` y habría descartado todas las respuestas en producción; verificado con 5 GET reales de solo lectura (`sonda-bulk.mjs`, misma carpeta que el resto del material fuente) y con tests que fallan si el prefijo se quita.
