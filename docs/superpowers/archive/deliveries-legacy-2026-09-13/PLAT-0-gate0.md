...
- Entrega / estado: **en curso** — cumplidos: backups B2 cifrados con vigía y heartbeat externo, disco, monitoreo externo, Node 24, QA bajo demanda. **Pendiente para cerrar P0:** PostgreSQL vacío con archivo continuo de WAL a B2, base backups verificados y restauración PITR medida (RPO ≤ 5 min, RTO ≤ 1 h). P1 no empieza hasta cerrar esto (revisión 2026-09-13: evita la dependencia circular P0↔P1).
...
- Próxima acción exacta y reproducible: escribir y ejecutar el paso "PostgreSQL + PITR" (contenedor PostgreSQL 18 por digest con límites, `archive_command` de WAL cifrado a B2 con clave sin borrado, base backup diario con `pg_verifybackup`, restauración a un instante en QA con RPO/RTO medidos y vigía de WAL sin archivar). Sin esquema de aplicación: eso es P1. Antes de ~8 GB en el bucket B2 decidir tarjeta, lifecycle o proveedor.
