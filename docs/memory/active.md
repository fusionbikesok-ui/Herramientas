...
- El worker se configura por entorno, todo o nada: `BARRIDOS_CUENTA`, `BARRIDOS_ML_URL`, `BARRIDOS_WOO_URL`, `BARRIDOS_ML_SELLER` y `BARRIDOS_KEYRING_FILE`; sin ellas no registra procesadores y no reclama nada. Atiende una sola cuenta y rechaza una corrida de otra. El keyring es JSON con claves de 32 bytes en base64, se rechaza si el archivo es legible por grupo u otros, y se monta como carpeta read-only (el contenedor corre con uid 1000; montar carpetas, no archivos sueltos).
...
Falta para cerrar E1: revisión independiente de T2 y después los tramos 3 (sombra en vivo) y 4 (seguridad y reporte). T2 verificado no acepta E1 y nada de esto está desplegado.
