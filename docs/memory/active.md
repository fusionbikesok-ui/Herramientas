...
Falta para cerrar T2: releer el segmento de `/orders/search` cuando cambia el `total` durante el paginado (decisión aprobada, todavía sin implementar; hoy una orden puede saltearse si otra sale de la ventana congelada); `worker/main.ts` registrando adaptadores por variables de entorno para una sola cuenta de ensayo (cuenta, URL de ML, URL de Woo, id de vendedor y ruta del keyring); el simulador como servicio del `compose.test.yml` con fixture en memoria; `E1_TRAMO=2 npm run test:e1` corriendo el worker real contra ese simulador; y actualizar `specs/e1/test-e1.md`.
