...
async function levantar() {
  ...
  const legacyPort = await freePort(); const app = express(); app.use(express.json()); app.use((q, _r, n) => { q.user = { username: 'qa-real', is_admin: true }; n(); }); app.get('/api/auth/me', (_q, r) => r.json({ ok: true, is_admin: true, usuario: 'qa-real', permisos: ['matcher'] })); app.use('/api/bandeja-identidad', bandejaIdentidadRouter({ url: `http://127.0.0.1:${apiPort}`, keyring: cargarKeyringInternoActivo(pgKey) }));
