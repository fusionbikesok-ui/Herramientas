...
const blockCodes = new Set(['FALTA_ENTORNO', 'FALTA_DATOS', 'FALTA_DECISION', 'PERMISO', 'FALLO_REPRODUCIBLE']);
...
export function untrackedBytes(cwd) {
  const names = git(cwd, ['ls-files', '--others', '--exclude-standard', '-z']).split('\0').filter(Boolean).sort(); const entries = [];
  for (const name of names) { const file = path.join(cwd, name); const stat = fs.lstatSync(file); if (stat.isFile()) entries.push(Buffer.from('file\0'), Buffer.from(name), Buffer.from('\0'), fs.readFileSync(file), Buffer.from('\0')); else if (stat.isSymbolicLink()) entries.push(Buffer.from('symlink\0'), Buffer.from(name), Buffer.from('\0'), Buffer.from(fs.readlinkSync(file)), Buffer.from('\0')); }
...
export function authoritativeGitState(cwd, refs = {}) {
  ...
  const untracked = untrackedBytes(cwd);
...
export function validateHandoff(handoff, role, { cwd } = {}) {
  ...
  if (h.estado === 'BLOQUEADO' && (!blockCodes.has(h.codigo_bloqueo) || !h.siguiente_accion)) throw new Error('bloqueado requiere codigo_bloqueo válido y siguiente_accion'); if (h.estado === 'WAITING_FOR_ORCHESTRATOR' && !h.siguiente_accion) throw new Error('waiting requiere siguiente_accion'); if (canonical === 'probador-e2e' && h.estado === 'BLOQUEADO' && (!h.evidencia || !Array.isArray(h.anchos_riesgos))) throw new Error('E2E bloqueado requiere evidencia y anchos_riesgos'); if (h.estado !== 'APROBADO') return h;
