...
function adaptador(paginas: readonly PaginaRemota[], fullScan = false, versionKind: TipoVersion = 'temporal'): AdaptadorBarrido {
  return {
    topic: 'ml.orders', fullScan, versionKind,
...
describe('motor transaccional de reconciliación E1 T2', () => {
  ...
  async function nuevaCorrida() {
    ...
    return (await reclamarCorridas(db, 'motor-worker', ['ml.orders'], 1))[0]!;
  }
  ...
  it('una falla en página 3 conserva cursor y el reintento repite la ventana congelada sin duplicar', async () => {
    ...
    const reintento = (await reclamarCorridas(db, 'motor-worker', ['ml.orders'], 1))[0]!;
