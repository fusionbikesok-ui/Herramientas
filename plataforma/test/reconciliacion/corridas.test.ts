...
describe('programación y leases de barridos E1 T2', () => {
  ...
  it('dos workers no reclaman la misma corrida y el lease se renueva', async () => {
    ...
    const [a, b] = await Promise.all([
      reclamarCorridas(db, 'w1', ['ml.orders'], 1),
      reclamarCorridas(db, 'w2', ['ml.orders'], 1),
    ]);
  ...
  it('el worker sólo reclama tópicos con procesador registrado', async () => {
    await corriente();
    ...
    const worker = crearWorkerBarridos({
      ...
      procesadores: { 'ml.orders': async () => ({ cursorAfter: { v: 1, updated_at: '2026-09-15T00:00:00Z', tie_breaker: '1' } }) },
    });
  ...
  it('conflicto optimista deja partial y lease vencido no escribe', async () => {
    await corriente();
    const corrida = (await reclamarCorridas(db, 'w1', ['ml.orders'], 1))[0]!;
    ...
    const otra = (await reclamarCorridas(db, 'w2', ['ml.orders'], 1))[0]!;
  ...
  it('reintenta, agota, recupera lease y devuelve intento al apagar', async () => {
    ...
    let corrida = (await reclamarCorridas(db, 'w1', ['ml.orders'], 1))[0]!;
    ...
    corrida = (await reclamarCorridas(db, 'w1', ['ml.orders'], 1))[0]!;
    ...
    corrida = (await reclamarCorridas(db, 'w1', ['ml.orders'], 1, 1))[0]!;
