...
import { getAccessToken, bootstrapToken, getEstadoRefresh, _resetEstadoParaTests } from '../lib/mlClient.js';
...
describe('mlClient', () => {
  ...
  it('getAccessToken: hace refresh si el token vence en < 30min (margen nuevo)', async () => {
  ...
  it('getAccessToken: 401 al refrescar es fatal (requiere re-autorización)', async () => {
    ...
    await expect(getAccessToken(db, ML_CFG)).rejects.toThrow(/re-autorización/);
