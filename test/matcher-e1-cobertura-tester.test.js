...
describe('Matcher unificado, entrega 1 — pausar/desvincular admin-only en el backend', () => {
  ...
  it.skip('POST /solo-ml/:clave/pausar sin ser admin responde 403 y no llama a ML', async () => {
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('POST /solo-ml/:clave/pausar CON admin sí procede (control: la guarda es real, no siempre 403)', async () => {
    seedMlSinSku(db, { clave: 'MLA1|', item_id: 'MLA1', titulo: 'Pub sin SKU' });
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('POST /multi-publicacion/:clave/desvincular sin ser admin responde 403 y no borra la decisión', async () => {
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('POST /multi-publicacion/:clave/desvincular CON admin sí procede (control)', async () => {
    seedProducto(db, { id_woo: 1, sku: 'FB-1', nombre: 'X' });
    db.prepare(`INSERT INTO sku_matcher_decisiones (clave, sku, wc_nombre, accion, origen, actualizado_en)
      VALUES ('MLA1|', 'FB-1', 'X', 'confirmar', 'cobertura', ?)`).run(new Date().toISOString());
    mlFetch.mockResolvedValue({ status: 200, data: {} });
...
describe('Matcher unificado, entrega 1 — deshacer exige ser el autor', () => {
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('un no-admin sobre un vínculo AJENO recibe 403 diciendo quién lo confirmó, y no lo toca', async () => {
    seedProducto(db, { id_woo: 1, sku: 'FB-1', nombre: 'X' });
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('el AUTOR (no-admin) sí puede deshacer su propio vínculo', async () => {
    seedProducto(db, { id_woo: 1, sku: 'FB-1', nombre: 'X' });
    seedMlSinSku(db, { clave: 'MLA1|', item_id: 'MLA1', titulo: 'X' });
    db.prepare(`INSERT INTO sku_matcher_decisiones (clave, sku, wc_nombre, accion, origen, confirmado_por, actualizado_en)
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('un vínculo SIN confirmado_por (anterior a la migración 013) solo lo deshace un admin: un no-admin recibe 403', async () => {
    seedProducto(db, { id_woo: 1, sku: 'FB-1', nombre: 'X' });
    seedMlSinSku(db, { clave: 'MLA1|', item_id: 'MLA1', titulo: 'X' });
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('un admin SÍ puede deshacer un vínculo ajeno (control: la guarda no bloquea de más)', async () => {
    seedProducto(db, { id_woo: 1, sku: 'FB-1', nombre: 'X' });
    seedMlSinSku(db, { clave: 'MLA1|', item_id: 'MLA1', titulo: 'X' });
    db.prepare(`INSERT INTO sku_matcher_decisiones (clave, sku, wc_nombre, accion, origen, confirmado_por, actualizado_en)
      VALUES ('MLA1|', 'FB-1', 'X', 'confirmar', 'cobertura', 'ana', ?)`).run(new Date().toISOString());
...
describe('Matcher unificado, entrega 1 — reasignar: concurrencia optimista y origen/accion correctos', () => {
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('reasignar escribe origen="cobertura" y accion="confirmar" — sin eso el vínculo no se puede deshacer, no cuenta en el progreso y no aparece en el historial', async () => {
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('reasignar: si otra persona ya vinculó esa clave a OTRO sku, revalida antes de escribir y responde 409 con quién y qué (no pisa en silencio)', async () => {
    sembrarVinculo(db, { clave: 'MLA1|10', sku: 'FB-1' });
    seedProducto(db, { id_woo: 2, sku: 'FB-2', nombre: 'Otro' });
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('reasignar: si la reasignación previa fue de la MISMA persona, el 409 vuelve con propio:true (distinto mensaje, misma revalidación — no se pisa a ciegas ni siquiera a uno mismo)', async () => {
    sembrarVinculo(db, { clave: 'MLA1|10', sku: 'FB-1' });
    seedProducto(db, { id_woo: 2, sku: 'FB-2', nombre: 'Otro' });
    seedProducto(db, { id_woo: 3, sku: 'FB-3', nombre: 'Tercero' });
    const app = appComo(db, { id: 1, username: 'ana', is_admin: 0 });
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('reasignar permite cambiar deliberadamente una decisión moderna si expected_sku coincide', async () => {
    sembrarVinculo(db, { clave: 'MLA1|10', sku: 'FB-1' });
    seedProducto(db, { id_woo: 2, sku: 'FB-2', nombre: 'Otro' });
  ...
  // esta cobertura vuelve a hacer falta. El contrato vigente lo fija
  // test/cobertura-legacy-retirada.test.js.
  it.skip('reasignar exige expected_sku para no convertir una lectura vieja en un pisado ciego', async () => {
    sembrarVinculo(db, { clave: 'MLA1|10', sku: 'FB-1' });
    seedProducto(db, { id_woo: 2, sku: 'FB-2', nombre: 'Otro' });
    const app = appComo(db, { id: 1, username: 'ana', is_admin: 0 });
